...
export async function initRBCurriculum(options = {}) {
  ...
  for (const [k, v] of Object.entries(theme)) {
    try { target.style.setProperty(`--${k}`, v); } catch {}
  }
  ...
  function attachQuizHandlers(m) {
    ...
    tabpanel.querySelectorAll('.quiz-option').forEach(opt => {
      ...
      opt.addEventListener('keydown', e => {
        if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); select.call(opt); }
      });
      function select() {
      ...
      submit.addEventListener('click', () => {
        ...
        if (score >= passMark) {
          ...
          if (!state.completed.includes(m.month)) {
            ...
            renderMonths();
            updateProgress();
            try { onMonthComplete({ month: m.month, score }); } catch {}
            target.dispatchEvent(new CustomEvent('rb:monthComplete', { detail: { month: m.month, score } }));
          }
        } else {
