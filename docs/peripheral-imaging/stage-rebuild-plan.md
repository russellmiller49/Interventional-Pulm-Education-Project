...
`/fluoroview`. The spine is the **imaging chain** — source → beam → patient → detector →
...
| #   | Decision                                                                                                                                                           | Why                                                                                                                                                           |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Nested routes `fluoroview/{page,learn,practice,assess}`; `/learn?section=<id>`; `/assess`                                                                          | Same shape as the critical-care modules. `/fluoroview` is in no public list, so the subroutes inherit the login gate.                                         |
| 2   | `src/i18n/path.ts` exempts `/fluoroview/(learn\|practice\|assess)` from the asset-root locale bypass                                                               | `/fluoroview/*` is also the legacy asset root; without the exemption a typed `/fluoroview/learn` fell through to the module-asset fallback rewrite and 404ed. |
...
- `npx jest src/features/peripheral-imaging 'src/app/\[locale\]/fluoroview' src/i18n/locale.test.ts --runInBand`
...
- `npx eslint src/features/peripheral-imaging src/app/\[locale\]/fluoroview e2e/peripheral-imaging.spec.ts`
...
- Owner items: delete or keep the orphaned legacy simulator (`src/components/fluoroview/`);
