...
- **Owner decision — value-guide coverage remains deliberately scoped.** B6 adds draft-needed guides for pump speed, external sweep, sweep-gas oxygen fraction, SpO₂, PaCO₂, pH, bicarbonate, post-oxygenator saturation, MAP, pulse pressure, native output, right-radial/femoral saturation, and battery reserve. Gas-source state, generic power-source state, bubble/reset state, and limb NIRS still lack dedicated guides. A separate audit must determine whether those and every other learner-facing number or hidden threshold need a guide, source class, or visible boundary.
