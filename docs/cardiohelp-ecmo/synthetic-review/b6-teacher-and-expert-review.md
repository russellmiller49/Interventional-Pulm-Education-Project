...
- **Confirmed defect — the curriculum hub overstates and bypasses the authored runway.** “Start curriculum” enters the drill sequence rather than the ten foundation sections, while hub copy describes a different count/shape from the 17-section pathway. Capstone prerequisite IDs use clinical drill IDs while the ordinary Practice path records Practice scenario IDs, so normal completion cannot satisfy the stated prerequisite contract even though the hub exposes the capstone.
...
- **Confirmed defect — tab interfaces implement roving `tabIndex` but no Arrow/Home/End behavior.** This affects foundation compact tabs and simulator bedside/pressure-view tabs. A keyboard-only learner can tab to the selected tab but cannot use the standard tab-list keys to change selection.
...
- **Confirmed defect — transformed console surfaces can shrink interactive targets and text.** `FitWidthSurface` scales the whole console; the Learn surface offers no actual-size alternative. Browser rendering must report the physical compact capture rather than inferring compliance from CSS pixel declarations.
...
- **Confirmed defect — one feedback surface combines `role="alert"` with `aria-live="polite"`.** The urgency contracts conflict. Multiple nearby live regions also create a screen-reader announcement-order hypothesis that still requires assistive-technology testing.
...
4. Can a keyboard-only learner discover and operate both tab interfaces without pointer rescue?
