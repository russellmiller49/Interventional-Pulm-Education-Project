...
| `SOCRATES_Steve_Followup_Pack/` | Original workbook, private staging, unresolved mapping checklist, source audit and `batch-01/` private bootstrap plans/packages | Read by absolute path with `scripts/socrates/{workbook-import,case-bootstrap}.ts`; never public or committed |
