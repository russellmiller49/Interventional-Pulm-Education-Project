...
| `/en/cardiohelp-ecmo` **after the merge**                             | Read the sources panel                                                                            | Badge "UNLISTED DRAFT · Clinical and device review: none recorded"; the same IFU-check, Publication ("Unlisted draft") and review ("None recorded") rows; five source groups; no "REVIEW APPROVED", "Reviewed release" or "Reviewed English" anywhere on the page; SHARED-02's "Unlisted tester access" frame label intact                                                                               |
| `…/learn?track=vv&lesson=vv-series-physiology` **after the merge**    | **Continue without doing this step** three times                                                  | The recirculating-share card reads "…On the reference circuit it stays at its baseline at every speed" at the act step, and the adjusted-flow value guide reads "On the reference circuit the fraction stays at its baseline, so this value rises with flow" at the explain step. The old "Each case sets where this starts" sentence is gone                                                            |
| `…/learn?track=vv&lesson=blood-flow-versus-sweep` **after the merge** | **Continue without doing this step** once                                                         | The saturation-ceiling note renders verbatim at the act step; the console sweep boundary names the 20 mmHg bound; "no plateau" appears nowhere; storage holds only `selfPaced` visited topics and location                                                                                                                                                                                               |

**Not seen in the browser:** the rewritten sweep-prediction explanation (ECMO-03-09, step 5) and the
VV series mixture model boundary (ECMO-03-10), which sits in the mixing-arithmetic block. The in-app
browser pane was hidden: the step outline sat outside its viewport at every emulated height, the
in-step Continue control is re-rendered on each step, and a `phase=` deep link reopens at the first
task by design (ECMO-01). Both are covered by the rendered and unit tests above. The
recirculating-share card, the adjusted-flow value guide, the saturation ceiling and the PaCO₂ bound
were all read in the browser after the merge.

## Integration with origin/main (2026-09-15)

`origin/main` was merged into `claude/ecmo-03` (merge commit `0b32c58f`, no rebase) after SHARED-02
(PR #225) and MV-SAFETY-01 (PR #226) landed at `a465b486`. Three files conflicted and were resolved
semantically and additively:

| File                                                       | Resolution                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ---------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/features/cardiohelp-ecmo/components/SourcesPanel.tsx` | Kept this batch's review line, which no longer follows the publication flag, and adopted SHARED-02's published-state vocabulary: the badge reads `PUBLISHED` or `UNLISTED DRAFT` beside "Clinical and device review: none recorded", and the Publication row reads "Public release" when published and "Unlisted draft" otherwise. The separate "Clinical and device review: None recorded" row and the IFU document-check row are kept. SHARED-02's removal of "REVIEW APPROVED" and "Reviewed release" stands |
| `docs/gap-remediation/self-paced/README.md`                | Both handoff entries kept, ECMO-03 then MV-SAFETY-01                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `docs/gap-remediation/self-paced/test-contracts.md`        | Main's reformatted migration table kept, with both the MV-SAFETY-01 and ECMO-03 rows                                                                                                                                                                                                                                                                                                                                                                                                                            |

Everything else came in unchanged: SHARED-02's other corrections (the ECMO module frame's "Public
release" and "English content fallback", its ECMO component test, the GEF attribution records in the
shared evidence registry and `sourceConflicts.ts`) and every MV-SAFETY-01 file. The ECMO-03 claim
queue's quoted badge line was updated to the resolved wording, and its previous-wording field now
records that SHARED-02 had already replaced the two published-state phrases on main.

| Check after the merge                                                                 | Result                                                                                                                                                                                             |
| ------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Current `origin/main` in a detached worktree, same jest scope as the baseline         | **112 suites (109 passed, 3 failed); 2,557 tests (2,554 passed, 3 failed)**                                                                                                                        |
| Merged branch, same scope                                                             | **114 suites (111 passed, 3 failed); 2,580 tests (2,577 passed, 3 failed).** The three failures are main's own, with identical messages; the only suite-count difference is the two ECMO-03 suites |
| `tsc --noEmit`; ESLint `--max-warnings=0` on the resolved panel and the dating module | Exit 0 and exit 0                                                                                                                                                                                  |
| Prettier on the resolved files; `git diff --check`                                    | Clean                                                                                                                                                                                              |
...
Full `npm test`; production build; Playwright; the sweep-prediction explanation and the VV series
mixture boundary in a browser (see above); the Practice case source list in a browser; keyboard-only
and screen-reader passes; narrow or tablet widths; Safari or Firefox; es and zh-CN wording (English fallback only); authenticated sync or
