...
| **F21** RV weakening vs the LV        | **Reproduced → inference contained; the coupling gap is a named OD-03 hold.** The original head claimed the displayed wedge has no right-sided term; the sanity review disproved that — there is a small indirect response through the LV-compartment contribution that rounds away at the reference comparison (20.4214 → 20.0727 mm Hg, both displayed 20). Copy now says only what the display resolves. |
...
| **F25** suction with a full LV        | **Reproduced → repaired for what the alarm claims; the predicate is an OD-03 hold.** Right-sided delivery was the limiter throughout the original head's sample, but the sanity review reached all three branches through supported controls, so that sample established no universal saturation. The alarm and panel now name whichever term is actually the minimum.                                      |
...
report found it. The learner-facing sentence says the control rescales the entire circulation and still carries no
figure that could be read as a dose — Prompt-01's guard is intact and tested. The original head
added that the response is larger than a bedside fluid challenge would produce; the sanity review
removed that comparison as an unsupported clinical claim, and the shipped sentence now says the
magnitude **cannot** be translated into a bedside fluid-challenge response at all. A right-ventricular-
...
| `content/unloadingExamples.ts`, `McsUnloadingComparison.tsx`       | Two value columns                                       | A matched-time difference column. The original head qualified it against idle-drift deadbands; the repaired head rounds the difference to display precision and, when it is zero, says "No resolvable displayed change" | 18 and 18 with no comment taught "Impella hardly unloads" (F24)                                                                                            |
...
| Suite                              | Assertion                                            | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---------------------------------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `unloading-comparison.test.tsx`    | Each signal row had exactly two cells                | The row has a third: the matched-time difference. The two value cells are still checked exactly as before — still the engine's numbers and nothing else — and the added assertion is that the third is arithmetic on those two. The sanity review then replaced the deadband qualifier with display-precision rounding, because comparing an already-rounded metric against an idle-drift band claimed a sub-resolution movement the data could not support. |
...
| -------------------------- | ---------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MCS feature                | `npx jest src/features/mechanical-circulatory-support`                                                     | Original head **41 suites / 839 tests**; **repaired head 42 suites / 844 tests, all passing**                                                                                                                                                                                                                                                                    |
| MCS + all app routes       | `npx jest src/features/mechanical-circulatory-support src/app`                                             | Original head **79 / 1238**; **repaired head 80 suites / 1243 tests, all passing**                                                                                                                                                                                                                                                                               |
| Independent invariance     | the sanity review's `probe.cjs`, base vs repaired head, seeds 417 and 913                                  | **61 plans, 1,422 exact comparisons, all passing** — re-run on this head against a fresh detached `c1fb8a0d` checkout                                                                                                                                                                                                                                            |
| Replay-arm timing          | same probe, `harness` section                                                                              | Control / one-action / two-action arms all end at **16.04 s**; the flag-only arm has identical modeled flow and **+2.8 W**                                                                                                                                                                                                                                       |
| Full suite                 | `npx jest`                                                                                                 | **10 suites / 9 tests failing** — and the **identical ten** on the base under a matched full run (see below)                                                                                                                                                                                                                                                     |
| Base comparison            | `npx jest` at `c1fb8a0d` in a detached worktree, same machine                                              | **10 failed / 9 tests** — the same ten suites, same names                                                                                                                                                                                                                                                                                                        |
...
| Latest-main integration    | disposable worktree from the repaired head with `origin/main` `a306d825` merged in (88 commits, 379 files) | **Conflict-free.** MCS + routes + `learning-module` + `hemodynamics-core`: **95 suites / 1382 tests passing**. `tsc --noEmit` clean. Invariance probe re-run on the integrated tree: **1,422 comparisons passing**. `npm run build` **exit 0**. Browser smoke of the hub, F17, F19 and F25 clean. The worktree was discarded; this branch was never merged into. |

**The ten pre-existing failures**, identical on base and head under matched full runs: the
brochure-intake static scan, the US-status safety boundaries, `training-apps.test.mjs` (no tests),
branch-tracing contracts, **branch-tracing lesson**, critical-care accessibility, critical-care
curriculum sequencing, critical-care learner copy, the Literature foundation manifest, and
board-review localized HTML. None is MCS, and this PR changes no file in any of them.

**One integration-only artifact, recorded rather than glossed.** The first two `npm run build` runs
on the disposable integration tree failed in the nested Vite sub-projects (`navigation_module/web`,
then `EBUS-course/apps/web`) with `Cannot find package '@vitejs/plugin-react'`. Those sub-projects
carry their own gitignored `node_modules`, which a fresh worktree does not have; `origin/main`
changed no file under either of them, and the same build is exit 0 on the real worktree at the same
head. Linking the nested dependency directories in produced a clean exit-0 build on the integrated
tree. It is a checkout-completeness artifact of disposable worktrees, not a regression from this
branch or from main.

`src/features/bronchial-branch-tracing/__tests__/lesson.test.tsx` is **load-sensitive**. The
original head's handoff recorded it as a one-off flake because it passed in isolation on an idle
machine; on this pass it failed three consecutive isolated runs while a second full suite was
running, then passed again once the machine was idle. A matched full run on the authorized base
fails it too, so it is a pre-existing timing sensitivity, not a consequence of this branch.
Corrected here rather than left as the earlier "nine".

### Browser journeys

Two passes. The original head was walked with the in-app browser pane on port 3124. The repaired
head was re-walked with **Playwright and headless Chromium** on port 3125, at 1440×1000 and 390×900,
with screenshots — the sanity review showed the pane's blank frames were a capture limitation of
that driver, not a page fault, so the earlier "screenshots could not be captured" note is superseded.
Console errors are the usual Supabase `500`s from the absent `.env.local`. The dev server was
stopped before the build.

| Route                                                | Observed on the repaired head                                                                                                                                                                                                                                                                                                        |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `learn?lesson=iabp-timing-triggering`                | **F17 clipping repaired.** viewBox `0 0 320 120`; **no SVG text overflows it** (the original head's second footer line reached 385.68 units in a 320-unit box). The two prose lines are now a wrapping HTML `figcaption`, fully visible at 390 px (256 px tall, no page overflow).                                                   |
| same                                                 | Authored reference: five landmarks, `not this patient's trace`, `not a run of this simulation`, the source's `ideally` preserved, and **no 15–20 mm Hg magnitude anywhere**                                                                                                                                                          |
| same, all five timing demonstrations                 | Each arterial trace sits at y 17.3–25.9 with height 29.0–31.1 in a 64-unit band — none fills it edge to edge, so all five are drawn on the common fixed 40–120 mm Hg domain and are comparable                                                                                                                                       |
| `learn?lesson=impella-unloading-placement`, P6       | Original head: wedge row `18 / 18 / +0 mm Hg · below this model's resolution`. **Repaired head: `No resolvable displayed change`**, with nonzero rows printing plainly (`−4 mL`, `+1.05 L/min`)                                                                                                                                      |
| same, after choosing P8                              | `118 / 107 / −11 mL`, `2.44 / 3.49 / +1.05 L/min`, wedge `18 / 17 / −1 mm Hg` — all unqualified, because all are resolvable                                                                                                                                                                                                          |
| same, 390×900                                        | **F24 compact reachability repaired.** The comparison table is not clipped and sits in a focusable labelled scroll region (`tabindex="0"`, `aria-label="Filled reference pressure and flow comparison"`)                                                                                                                             |
| `learn?lesson=impella-suction-purge-rv`, step 1      | `Left Impella suction detected` beside wedge **20.0** and LV volume **128.0 mL**, with _Smallest term feeding the left inlet · right-sided delivery to the left heart · 0.43 on this model's **dimensionless scale (terms can exceed one)** … a value below 0.58 raises modeled suction_                                             |
| same, story problems                                 | **F26 repaired.** The scope sentence now says the magnitude _cannot be translated into a bedside fluid-challenge response_; the original head's "larger than a bedside fluid challenge" comparison is gone, and no mL figure appears                                                                                                 |
| `learn?lesson=iabp-efficacy-limits`, step 1          | **F21 repaired.** The distractor feedback names the small indirect response and refuses the generalization: _the displayed wedge has a small indirect response through the LV compartment contribution, but it rounds away here. This does not show that wedge pressure cannot change with RV failure._                              |
| `learn?lesson=lvad-alarms-emergencies`, step 2       | **F27 repaired.** Afterload row now names **both** factors: _the modeled afterload multiplier reduces flow by 10% from otherwise identical modeled loading (the minimum of unsupported-baseline and compartment-gradient factors, not a measured device quantity)_, and the alarm's own input is 67 mm Hg against a threshold of 100 |
| same                                                 | **F28:** the durable flow identity renders and the panel says _modeled transfer_, not an estimate                                                                                                                                                                                                                                    |
| `learn?lesson=mcs-device-selection-integration`      | RAP **17.0**, wedge **18.0**, the filling-pressure congestion panel present, the reworded key, and **no occurrence of "modest" on the page**                                                                                                                                                                                         |
| `practice` → LVAD-02                                 | **F35 repaired.** Both criteria shown; the debrief identifies a _worked comparison_ and explicitly disclaims an action the learner took                                                                                                                                                                                              |
...
Screenshots for each row are in the session evidence folder, alongside the sanity review's own
desktop and mobile captures. They are local evidence, not clinical approval.

**Not run:** native browser zoom, Safari or Firefox, a real phone or touch device, assistive
technology, `es`/`zh-CN`, authenticated sync, the 1280×800 / 768×1024 / 320×740 widths and 200% root
text on every surface, and any learner observation. Those are OD-08 and separately executed
engineering.
