...
**Review-corrected disposition (September 22): all 37 Teaching ledger IDs are present exactly
once. 32 are repaired (including explicitly provisional vocabulary), and 5 retain an owner-held
portion: 1.3, 2.6, 2.7, 3.2 and 5.4. Zero are already resolved or not reproduced.** Safe parts
of these five are implemented. This supersedes the original 33/4 accounting: the original table
actually marked 34 repaired and 3 held; its prose counted 2.6 and 2.7 inconsistently. The independent
review additionally treats CBCT wording confirmation (5.4) as a substantive hold.

Six packets are not six distinct ledger rows: rEBUS spans 1.3/3.10; signed angles spans 2.6/3.2;
pulse ownership is 2.7; CBCT provenance is 5.4; VESPA wording choices are 6.5; the drafted definitions
belong to CW3 (binning also appears in 3.3; stored contour in 2.14). The latter two packets are an
editorial choice around sourced wording and explicitly provisional definitions, respectively.
See [independent sanity review](PI-FELLOW-03-sanity-review.md) and the corrected
[text map](PI-FELLOW-03-text-map.md). No owner decision was taken by the technical reviewer.
...
| **2.6**  |       18 | Section 2, C-arm controls                                  | **Held for the owner** (safe labels done)              | Slider said "C-arm obliquity", monitor said "Orbit 30°", caption said "Orbit and tilt". One name now; the model's sign convention is stated in patient terms and pinned by a geometry test. LAO/RAO not inferred. [Owner packet](PI-FELLOW-owner-decisions.md#26--32-lao-rao-and-cranial-caudal-labels)                                 |
...
| **5.4**  |       40 | Section 14, provenance flow                                | **Held for the owner** (safe modality correction done) | The mobile CBCT section rendered Section 11's DTS provenance flow ("limited-angle projections"). The flow now takes its modality from the section; the CBCT wording is drawn from Section 12's own account. Confirmation requested in the packet.                                                                                       |
...
Counts: **32 repaired**, **5 held for the owner with safe parts applied**, **0 already resolved**,
**0 not reproduced**. CW3 is repaired as provisional delivery, not clinical approval of its two drafts.
...
wording; (5) 6.5 the VESPA clause; (6) the two drafted definitions. These decisions remain open. Technical tests establish delivery and state preservation, not
source approval. Pulse ownership and CBCT provenance are visibly provisional; signed clinical-angle
mapping is not applied; VESPA effect-size additions are not applied.
...
## Independent review corrections — September 22, 2026

The original implementation evidence above is historical and is not counted as independent review
validation. The dedicated Codex review found and repaired: false personal-history wording in optional
review; unmarked pulse ownership and CBCT provenance drafts; a binning gloss outside the draft label;
a new categorical prior-CT exclusion absent from the packet; stronger reconstruction wording without
an identified local passage; loss of the free-air model qualifier in the KAP summary; and a Help test
filter that also skipped visible disclosure summaries. The review preserves the two glossary draft
labels, restores existing source wording where appropriate, and adds native keyboard opening coverage.
No clinical question was adjudicated. Exact checks and the original-head findings are in the independent
review, with failed runs separate from successful reruns.

The independent screenshot review additionally repaired the clipped Section 1 sampling label and
phone reconstruction-table word fragmentation. The label annotation was centered without moving any
anatomy, and the comparison now stacks by aspect/modality at phone widths. All six requested viewport
conditions were rechecked with the repaired surfaces.
