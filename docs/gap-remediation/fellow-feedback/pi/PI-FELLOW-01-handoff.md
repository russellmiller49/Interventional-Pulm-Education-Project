...
Implementation: September 19, 2026. Presentation-truth correction: September 20, 2026. Prepared by
Claude (AI implementation).

**Result: of the eleven assigned findings, ten were reproduced on the current build and repaired,
and one was already resolved.** Two further observations were made along the way and are recorded
[separately below](#additional-observations); they are not assigned findings and are not counted in
that total.

The change is PI-local: the fixed-example state, the stored-frame acquisition/display boundary, the
evidence framing of three checks, five copy surfaces, one optional retry path, one CSS rule for a
state the markup already reported, and one navigation link. No clinical claim, source record,
question id, choice id, answer key, rationale, authored example state, progress key, release flag,
clinical-review hold or storage schema was changed; no image was generated or replaced; no learner
state was reset; and nothing about the self-paced contract was relaxed.
...
- Browser work ran against a **local development build** on `127.0.0.1:3126`
  ...
  batch 05.
- **EBUS PR #249 had not merged** when this correction was finished (re-checked 2026-09-20 after the
  work): it is still `OPEN`, and `origin/main` is still `77a141cc`, so there was nothing to
  integrate and this branch is one commit ahead and none behind. #249 changes no file this branch
  touches, but it does change the **shared** `learning-module/components/AnswerVerdict.tsx`, which
  every Peripheral Imaging check renders. Compatibility was checked rather than assumed: with
  #249's `AnswerVerdict.tsx` overlaid on this branch, **every Peripheral Imaging suite passed**
  (53 of 54 suites, 440 tests). The single failure was that component's own test, which #249
  replaces in the same PR. Its change is additive — a conditional disclosure heading plus new
  `data-other-answer-role` / `data-other-answers-keyed` attributes — and leaves
  `data-answer-verdict`, `data-verdict-outcome` and `data-other-answers`, which are the only
  contracts PI asserts on, unchanged. Whichever of the two merges second needs no rework.
...
Neither is one of the eleven assigned findings, and neither is counted in the disposition above.
The first is now corrected as far as presentation goes; the second is recorded and not acted on.

1. **Three checks claimed their image depicted a scenario it does not** — `current-anatomy`,
   `changing-anatomy`, `staff-protection`. **Presentation corrected** on 2026-09-20; see
   [section 3](#3-presentation-truth-on-the-three-illustrative-checks). The remaining question —
   whether those stems should instead get an image they can be read from — is still an
   **owner/content decision**, overlapping report 2.11, and belongs to batch 03/04.
...
## 3. Presentation truth on the three illustrative checks

Added 2026-09-20, after the first review of this PR. A bounded correction of what the module
_claims_ about an image, not of the image, the question or the answer.

### The claim that was wrong

PI-FELLOW-01 made every check's image stable (section 1). Three of them then still carried, over a
written clinical scenario, the banner:

> This example stays fixed so the question and the image match.

and the instruction "Inspect the image and acquisition context." Their stems describe things their
section's model does not represent, as each section's own `modelBoundary` already states:

| Check                        | Its stem describes                                                                         | What the model actually simulates                                                                        |
| ---------------------------- | ------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------- |
| `current-anatomy:example:0`  | "a new dependent opacity now obscures a previously distinct peripheral lesion"             | a rigid CT translation the learner sets — "no ventilation, recruitment or a clinical intervention"       |
| `changing-anatomy:example:0` | "duplicated tool and lesion edges from motion during the spin"                             | the same rigid translation — "no ventilation, recruitment, motion artifact or vendor registration"       |
| `staff-protection:example:0` | "a clinician proposes holding an accessory in the primary beam while wearing a lead glove" | distance, tube side and a barrier — "not a staff-dose calculator, a room survey or a safe-distance rule" |

The image was fixed, authored and honest about being a model. The two sentences around it were not:
they told the learner to read an answer off a picture that does not show the situation.

### The correction

An image's relationship to its question is now **declared per question and round**, by the fixed
example's identity, in `content/teachingExamples.ts`:

- `fixedExampleEvidence(section, round)` returns `depicts-the-question` or `illustrative-model`.
- `ILLUSTRATIVE_ONLY_EXAMPLES` holds exactly three identities —
  `current-anatomy:example:0`, `changing-anatomy:example:0`, `staff-protection:example:0` — each
  commented with the stem and the model that does not represent it.

Nothing is keyed by section alone, so a section's other activities and its transfer round are
untouched, and no blanket rule can mislabel an activity whose image is its evidence.

For a declared check, and only for it:

- the banner becomes "This image stays fixed while you read it. It is this section's teaching model
  of the equipment, not a picture of the situation in the question — answer from the written
  scenario."
- the instruction becomes "Answer from the written scenario below. The image beside it is this
  section's teaching model of the equipment, not a picture of the situation described."
- the pane reports `data-example-evidence="illustrative-model"`.

**Preserved exactly:** the question, its options and their order, the answer key, the rationale and
verdict, the authored example state and the image drawn from it, the section's `modelBoundary`, the
clinical-review holds, and every self-paced affordance (show the explanation before answering,
retry, continue without answering). **No replacement image was generated and no learner state was
reset.** Whether these stems should instead get an image they can be read from remains the open
owner/content decision recorded above.

### Held to three

Checks whose image really is the evidence keep saying so, and are asserted as controls:
`chain-walk:example:0`, whose image does superimpose the tool and the nodule its stem describes, and
`cbct-acquisition:example:0`, whose lateral scout does show the offset its stem describes. Both keep
"the question and the image match" and "Inspect the image and acquisition context". Every transfer
round resolves to `depicts-the-question`, and the declared list is pinned by test.
...
| `src/features/peripheral-imaging/__tests__/truthful-surfaces.test.ts` (new, 12 tests)                 | 1.2, 1.4, 8.1, 8.2, PR2; the declared evidence relationship of every check and round, held to exactly three illustrative identities; and the two recorded inventories (1.7's no-control sections, the one remaining "Choose" reading screen).                                                                                                                                         |
...
| `src/features/peripheral-imaging/__tests__/example-evidence-framing.rendered.test.tsx` (new, 9 tests) | The presentation-truth correction, rendered: each of the three declared checks names its image as a teaching model, sends the learner to the written scenario, drops the matching claim, and keeps its stem, options, image, answer key, rationale, model boundary, explanation, retry and skip; `chain-walk` and `cbct-acquisition` are held to the image-based framing as controls. |
...
| Command                                                                                                                                             | Result                                                                                                                                                              |
| --------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `node node_modules/jest/bin/jest.js src/features/peripheral-imaging src/features/learning-module 'src/app/[locale]/peripheral-imaging' --runInBand` | **55 suites, 450 tests passed** (was 50/412 at G02).                                                                                                                |
...
| The same jest selection with #249's `AnswerVerdict.tsx` overlaid                                                                                    | **53 of 54 suites, 440 tests passed**; the one failure is that component's own test, which #249 replaces. No PI suite was affected.                                 |
...
| `NEXT_PUBLIC_SUPABASE_URL=… NEXT_PUBLIC_SUPABASE_ANON_KEY=… npm run build`                                                                          | **Exit 0**, zero errors. Run with this worktree's dev server and watchers stopped and port 3126 free. Invalid preview values; no stored credentials, no deployment. |
...
- **No image was generated, replaced, regenerated or re-authored.** The presentation-truth
  correction in section 3 changes two sentences around three images and nothing else: the stem,
  options, order, answer key, rationale, authored example state, drawn image, `modelBoundary` and
  clinical-review holds of all three checks are exactly as they were, and no learner state was
  reset.
- **The evidence relationship is declared per question and round, never as a blanket rule.** Three
  identities are declared; a test pins that list, holds every transfer round to
  `depicts-the-question`, and holds `chain-walk` and `cbct-acquisition` to the image-based framing.
...
- The `before/` evidence set was produced by reverting the 2.1, 2.12 and 4.5 repairs only; the O1,
  ...
  document and were captured before any edit. Section 3's before/after is held by its rendered
  regressions rather than by a browser capture: each of the three fails with the declaration
  removed, which is how that reversal was checked.
- Section 3 corrects what the module _claims_ about three images. It does not decide whether those
  three stems should instead get an image they can be read from — that remains an open
  **owner/content decision**, overlapping report 2.11 and belonging to batch 03/04. The wording
  itself has had no clinical or editorial review.
- The shared activity title "Interpret a changed example" is unchanged on all nineteen checks,
  including the three illustrative ones. It is generic, it is not a claim about evidence, and it is
  authored once for every section, so renaming it for three was out of scope for a bounded
  correction. Recorded here rather than changed.
- Nothing in this batch resolves report 2.11, the image-evidence matrix (PR1/IC3), or any item
  assigned to batches 02–05.

## 7. Files changed

Application:

- `src/features/peripheral-imaging/content/teachingExamples.ts` — the authored fixed-example table and its total resolver (2.1); the per-question evidence declaration (section 3)
- `src/features/peripheral-imaging/components/stage/ImagingStageHost.tsx` — use the resolver with no fallback; tag the example and its evidence relationship; the held-example banner; offer the sort retry (2.1, 1.10, section 3)
...
- `src/features/peripheral-imaging/content/stageLessons.ts` — truthful check and debrief prompts, including the illustrative-model instruction (1.2, section 3)
...
Tests: the five new files above, plus `stage-session.test.ts` and `e2e/peripheral-imaging.spec.ts`.
...
| Section 3         | `/en/peripheral-imaging/learn?section=current-anatomy`, `…=changing-anatomy`, `…=staff-protection` | Continue to the check. Before: "This example stays fixed so the question and the image match" and "Inspect the image and acquisition context". After: the image is named as this section's teaching model and the learner is sent to the written scenario. |
| Section 3 control | `/en/peripheral-imaging/learn?section=chain-walk`, `…=cbct-acquisition`                            | Continue to the check. Both keep the matching claim and the image-based instruction, because their images do show what their stems describe.                                                                                                               |
