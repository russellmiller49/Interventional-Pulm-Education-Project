...
2. **Provisional graph / reference samples.** The response point is a point **on** the daughter's
   centreline, but it is **not one of the graph's own samples**. The exporter walked a fixed
   **5.000 mm** along each daughter centreline from the node and linearly interpolated between the
   two graph samples that bracket that distance (edge 1 brackets k≈392.16 and k≈381.68; edge 2
   k≈392.16 and k≈378.92; interpolation error < 0.0001 mm). The resulting z, k≈387.15 and k≈386.87,
   both display on slice 387. The rule is uniform across the export: of the 195 daughter response
   points whose edges ship in `paired-routes.json`, **192 are interpolated and only 3 coincide with
   a graph sample**, no distance exceeds 5.000 mm, and 82 sit exactly at that cap (the shorter ones
   are on daughters whose own edge is shorter than 5 mm). Parent response points use the same
   construction with an 8.000 mm cap. Slice, patient-space point and native pixel are mutually
   consistent for every one of them (`slice === round(k(lps.z))`, `pixel === lps→pixel`).
   These remain centreline points, not lumen boundaries — as `local-exercises.ts` already states.
...
**No.** Four checks, all covered by tests. The HU check below compares two artifacts the exporter
produced from the same volume, so on its own it could in principle be self-consistent under a
uniformly wrong mapping; the anatomical anchors in the last bullet are what rule that out.
...
- **Anatomy anchors (independent of the transform being checked).** At a −900 HU airway threshold,
  both response pixels fall inside **one and the same** connected air component of roughly
  1000–1600 px on every plane from 394 down to 377, and that component becomes **two separate
  components at 374–372**. Separately, 198 of the 199 graph points in slices 360–414 land on air
  below −800 HU. And the single tracheal lumen carrying the parent point sits at slice 408, a
  **higher** index than both the response plane and the split — so higher indices are more cranial,
  the Z sign is not reversed, and the split is genuinely below the browsable interval. Pixel values
  are not derived from the graph coordinates, so this is an image-anchored check, not a restatement
  of the mapping.
- **Wiring.** The response planes are not hand-typed constants. They are read from
  `checkpoint.decision.options[i]`, whose slice/lps/pixel agree with each other and with the
  interpolated crossing of that daughter's own edge, and which are generated by the uniform 5 mm
  rule described above. The browsable interval 384–411 follows mechanically from those planes plus
  the ±3-slice margin.

So **387 is not a mis-wired number.** It is the point a uniform 5 mm-from-the-node rule lands on,
on a scan where the carina has not yet divided the column at that level. The plane was therefore
**set by a generic authoring rule that knows nothing about where the carina becomes visible** — it
is neither an indexing accident nor a per-junction anatomical judgement. Replacing it with 375, or
extending the interval toward 372, would be choosing a new anatomical response point, or changing
the rule for every junction — exactly what this task is not authorised to do, and what OD-01 exists
to decide.
...
- Nothing about the 5 mm rule, the response planes or any coordinate was changed. It is recorded
  here because it is what the owner decision in §5 actually turns on.
...
1. Is slice 387 — the plane a uniform 5 mm-from-node rule produces — intended as a proximal
   model-reference location rather than a visible two-lumen bifurcation? If so, should the exercise
   keep asking for a mark there at all?
...
3. Should the local lesson change only its demonstration, also its response planes, or be replaced
   by another already-verified fork? Note that the 5 mm rule is uniform across the whole export, so
   a change here is either a per-junction exception to it or a change to the rule itself.
