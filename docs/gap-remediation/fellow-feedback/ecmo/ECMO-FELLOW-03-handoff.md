...
request. The drawings no longer say false things:
...
- **20 are repaired**;
- **4 are partly repaired** (S2-2, S2-8, S7-2, VA11-1), with the remaining subpart and its owner
  named.
...
| S6-1   | P2  | R     | A VV series-loop schematic: seven nodes, a distinct dashed recirculation short-circuit, and "Schematic · not to scale" with a caveat. It sits before the existing text list, which is kept.                                                                                                                                                                                                                     | Clinical review of the schematic: **05** (teaching adaptation).                                                           |
...
| S17-1  | P1  | R     | The capstone matrix shows every hypothesis: a table at ≥1280, one card per hypothesis at ≤1024. A hypothesis filter can never hide the last hypothesis, and "Show all" restores everything. Visible columns 0 → 4 at 1280 (1 → 4 at 1440). No response gate.                                                                                                                                                    | —                                                                                                                         |
...
| VA7-3  | P2  | R     | The aortic-streams diagram is introduced in Section 6, where it is taught, and reused in the VA11 drill.                                                                                                                                                                                                                                                                                                        | —                                                                                                                         |
...
**Counts:** 20 R and 4 P (S2-2, S2-8, S7-2, VA11-1).
...
- **Software merge readiness:** ready for review.
  - Every gate above passes on the merged tree. The only failures are the three pre-existing
    consumer tests (identical on clean main) and the environmental 500s (identical on baseline).
- **Clinical, device and release readiness: not established by this batch.**
  - The dispositions are layout and teaching-visual repairs.
  - The two schematics and the relabelled map legend need clinical review.
  - The 2D/3D laterality question needs a content decision.
  - Nothing here validates physiology or the device.
