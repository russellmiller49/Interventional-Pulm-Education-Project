...
correction pass added fourteen more deliberate defects covering the congestion framework and every
reworded clinical claim, each confirmed to fail for its intended reason and then restored — twenty-two
in total across the two passes.

The fourteen, in the order they were run: the 0.85 ratio classifier; the 0.60 ratio classifier; the
ACC grid labelled a validated treatment algorithm; the Ortega cut points used as the primary
framework; the neither-elevated category called euvolemic; a congestion category recommending a
device; pulsatility index as the fraction of the cycle the native ventricle contributes; the
“exactly as it is on the real controller” claim; the “oxygen-delivery evidence” heading; the claim
that three ladder rungs are populated; the unqualified venovenous statement; the universal
venoarterial statement; a right atrial pressure alone marking the preload domain a positive finding;
and the old suction aphorism.
