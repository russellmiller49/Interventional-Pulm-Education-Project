...
The persistent map is the **catheter map** (`components/catheter-map/`): the existing right-heart
...
| 2   | Four places, four shapes (`waveform-interpretation`)               | mechanism   | 18  | each chamber writes its own shape, and the shape names the place                 | 1–4        | **the walk** — four stops on the catheter map, each with its tracing on the monitor; then name the place from a faulted display (**answered on the map**) |
...
clinical-hemodynamics waveform text) and authored teaching constructs (the catheter map, the five-thing
