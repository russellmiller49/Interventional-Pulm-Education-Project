...
> **Amended 2026-09-07 by [`mv-learner-review-record.md`](./mv-learner-review-record.md),
> MVLR-OD-1.** That order is now **learner action → teaching explanation → live
> ventilator/waveforms**: Steps, Teaching, Simulator, left to right, the arrangement the other
> three adopters of the shared lesson stage settled on after the September 2026 learner review.
> What this section was protecting is intact and is why the fractions are what they are — the
> console facsimile is never scaled, and the ventilator pane is still the widest of the three at
> every width validated in §3 and §5, re-measured there. What changed is which end of the row it
> sits at. The diagram above reads Ventilator | Teaching | Your turn; it is Your turn | Teaching |
> Ventilator.
...
| Evidence and task coexist; pane order; measurements with the console; pause outside the panes in every phase; one pause control; Help answers in the pane in every phase; Help does not hide pause; the verdict does not move the evidence; the debrief is a pane; no pane is remounted across a phase change; identifiers unchanged | `__tests__/learn-workspace.test.tsx` (12 tests) — **retired with the flow rebuild (PR #127)**, which replaced the surface it drove, so the pane order this row claimed to guard had no guard at all between then and 2026-09-07. It has one again: `__tests__/stage-learner-review.test.tsx` pins `['task', 'teaching', 'simulator']` and the fractions and floors that keep the ventilator widest — **the order itself changed with that guard**, see MVLR-OD-1 in [`mv-learner-review-record.md`](./mv-learner-review-record.md), which also re-measures §3 and §5 on the stage. The other claims in this row have no current guard. |
