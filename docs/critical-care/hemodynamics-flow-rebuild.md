...
| `9217995d` the tests        | The new stage suites; the H0–H5 suites kept for their content and re-pointed away from the deleted components; two suites that pinned only the deleted activities retired.                                                                                                                                     |
| the door, hydrated          | The Learn record read through `useSyncExternalStore` with the empty record as the server snapshot, and a hydration-safe `hydrated` flag; the Continue call to action renders `pending` on the server and on the hydrating render, `resolved` after, so the server markup and the first client render agree.    |
...
| Predict   | Commits before anything moves. The docks lock while deciding; the verdict states the outcome. Section 3 answers both its prediction and its transfer on the catheter map, with the monitor's chamber label withheld and the tip not drawn.                                                                                                                         |
...
- The 3D heart on the stage. The catheter map is where answers go; a drawing the preview pane
  ...
  advancement and wedge sections, where it sits beneath the docks as a teaching model — not an answer surface — and
...
| Check                                                                  | Result                                                                                                                                                                                                                                                                                                                  |
| ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Protected scope (the command in §7)                                    | 69 suites, 723 tests, all passing                                                                                                                                                                                                                                                                                       |
| Full `npm test`                                                        | 723 suites passing, 2 skipped; one failure outside the module, `scripts/ip-preference-cards/check-brochure-intake-static-exposure.test.ts`, which asserts an empty stderr from the tsx CLI and receives Node 26's `module.register()` deprecation notice — the test names Node 20; the file is untouched on this branch |
| `npx tsc --noEmit`                                                     | Clean, save the pre-existing contentlayer, board-review and mdx-utils errors on `main`                                                                                                                                                                                                                                  |
| `npx eslint` over the module, the stage package and the shared catalog | Clean                                                                                                                                                                                                                                                                                                                   |
| `npm run test:a11y`                                                    | 18 of 18                                                                                                                                                                                                                                                                                                                |
| The browser                                                            | Every one of the nine sections driven on the dev server through its completion card (the derived section's workbench to its Act step); the Overview and the Learn landing hydrate without a mismatch after the record hook moved to `useSyncExternalStore`                                                              |
