...
```mermaid
flowchart TD
A[New pleural effusion] --> B{Clear transudative cause e.g., CHF with response to therapy?}
B -->|Yes| B1[Consider no tap; treat cause Tap if atypical/poor response]
B -->|No or atypical| C[Bedside ultrasound size, complexity, loculations]
...
F -->|Parapneumonic/empyema| G[Start antibiotics + small-bore tube if complicated; consider tPA/DNase if poor drainage]
F -->|Malignancy suspected| H[Send cytology; symptom-guided pathway: IPC vs talc pleurodesis vs repeat taps]
...
C -->|Primary, symptomatic| D[Small-bore tube or aspiration with ambulatory device select pts]
