...
TBLC basics: Perform under general anesthesia via rigid bronchoscope or endotracheal tube; pre‑deploy bronchial blockers; fluoroscopy guidance is standard. Freeze times typically ~6 s (2.4 mm probe) or ~8 s (1.7 mm). Aim for samples with largest diameter ≥5 mm.
...
TBNA (central + hilar/mediastinal + some PPLs): Submucosal or extrinsic compression, peribronchial disease, necrotic lesions, nodal disease. Contraindications: Mirror general bronchoscopy; avoid vascular puncture sites and mucosa involved by tumor when staging to limit contamination.

TBLC

Indications: ILD/DPLD when CT + clinical + BAL are insufficient for confident diagnosis; transplant surveillance or selected critical‑care scenarios by experienced teams.
Absolute contraindications: Coagulopathy/thrombocytopenia and hemodynamic instability.
Relative contraindications: FVC &lt;50% predicted, DLCO &lt;35% predicted, pulmonary hypertension (sPAP &gt;40 mmHg), PaO₂ &lt;55–60 mmHg, significant emphysema, rapid clinical/radiographic deterioration suggestive of AE‑ILD.

Pre‑procedure Evaluation

All techniques: History/physical, CT review (identify bronchus sign, vessels, emphysema/cysts), oxygenation and PFTs, coagulation profile if risk factors present, consent addressing bleeding/pneumothorax and escalation (rigid bronchoscopy, SLB).
...
TBLC: Cryounit (CO₂ commonly used), disposable cryoprobes (1.1/1.7/2.4 mm); bronchial blockers (Fogarty 4–6 Fr or blockers through ET tube external channel); rigid tracheoscope or large ETT; fluoroscopy (CBCT/radial EBUS/other guidance optional).
...
Aim for ≥6 total specimens across tools.
...
Bleeding (central/PPL): Lateral decubitus bleeding side down; suction; cold saline; epinephrine (1:10,000) in 2–3 mL aliquots (≤3 doses); inflate Fogarty; escalate to APC/laser through rigid bronchoscope if ongoing.
...
Answer: TBNA of the submucosal lesion.

Explanation: TBNA penetrates deep layers/peribronchial tissue and provides high yield with less bleeding than forceps.
...
Explanation: Using TBNA first minimizes the impact of blood on cytology and is less affected by superficial bleeding; then add forceps.
...
Answer: TBLC with pre‑placed bronchial blocker.

Explanation: TBLC provides large, artifact‑sparing tissue samples with ~80% yield; blockers reduce major bleeding risk.

Question Bank

A 72‑year‑old with a 3 cm right upper lobe mass and obvious endobronchial tumor undergoes forceps biopsy ×2 yielding necrosis. What is the next best step to maximize diagnosis? Answer: B. Explanation: Necrotic tumors require deeper bites; sampling until surface bleeding often indicates viable tissue has been reached.
...
During central forceps biopsy, moderate bleeding obscures the field. Which immediate maneuver most effectively prevents contralateral soiling? Answer: B. Explanation: Lateral decubitus with the bleeding lung down plus continuous suction helps protect the non‑bleeding lung.
...
For bronchoscopic staging of right paratracheal nodes, which factor most improves TBNA yield? Answer: C. Explanation: Yield improves up to about four passes per target; more than that offers marginal gains.
...
Which scenario most strongly favors TBNA over forceps biopsy as the primary tool? Answer: B. Explanation: TBNA penetrates submucosa/peribronchial regions and is ideal for extrinsic compression lesions.
...
In fluoroscopy‑guided sampling of a 2.5 cm PPL with a clear bronchus sign, which tool sequence best preserves yield? Answer: B. Explanation: TBNA first minimizes adverse effects of bleeding on subsequent cytology/histology.
A. Forceps → TBNA → brushing
B. TBNA → forceps → brushing
C. Brushing → forceps → TBNA
D. Washing → forceps only
E. Forceps only
Which TBLC maneuver most reduces major bleeding risk? Answer: C. Explanation: Prophylactic blockers permit immediate occlusion of the target airway during and after specimen extraction.
...
Which patient characteristic most strongly argues caution or alternative to TBLC due to pneumothorax risk? Answer: B. Explanation: Low FVC is associated with higher pneumothorax risk; cystic patterns also raise risk, but low FVC is a consistent red flag.
A. Normal FVC, cystic CT pattern
B. FVC &lt;50% predicted without cysts
C. DLCO 40% predicted
D. Elevated BNP
E. Age &gt;80 years
A 64‑year‑old with suspected HP remains unclassified after HRCT and BAL. You plan TBLC. Which is the most appropriate procedural plan? Answer: B. Explanation: Standardized TBLC emphasizes blockers, near‑pleural positioning, multiple adequately sized cores, often sampling two segments.
...
For central airway biopsy in a patient on antithrombotics, which statement is most accurate? Answer: C. Explanation: Clopidogrel is typically held for ~7 days; many centers continue low‑dose aspirin; DOAC holds are generally 48–72 h.
...
While performing TBLC, the probe repeatedly catches at a lobar carina and cannot reach the periphery. Best next step? Answer: C. Explanation: Central biopsies risk bleeding and low alveolar content; reposition to obtain near‑pleural samples.
...
Which factor most strongly predicts success of the conventional bronchoscopy approach to a small PPL? Answer: B. Explanation: A bronchus leading to/within the lesion is a key predictor of transbronchial diagnostic success.
A. Lesion SUV on PET
B. Bronchus sign on thin‑section CT
C. Presence of calcification
D. Patient age
E. Lesion density (ground‑glass vs solid) only
Which statement about TBLC complications is most accurate? Answer: B. Explanation: Major bleeding risk falls substantially with routine blocker use; pneumothorax rates vary across studies (pooled ~5%); outpatient performance is feasible in many centers.
...
ILD/DPLD: (Diffuse) interstitial lung disease
