...
describe('the stage session', () => {
  ...
  const observe = lesson.steps.find((s) => s.interaction.kind === 'observe')!
  ...
  it('does not perform the lab step until its goals are met, and keeps it performed afterwards', () => {
    let session: ImagingStageSession = emptyImagingStageSession(lesson)
    session = reduce(session, { type: 'CONFIRM_THROUGH', index: actIndex - 1 })
    ...
    session = reduce(session, { type: 'CONFIRM_THROUGH', index: actIndex })
    ...
    expect(deriveStageProgress(lesson, session).liveIndex).toBe(actIndex + 1)
    expect(stepWorkDone(lesson, observe, observe.ordinal - 1, session)).toBe(true)
  })
