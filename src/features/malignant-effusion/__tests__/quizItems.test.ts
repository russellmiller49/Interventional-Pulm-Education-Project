import { mpeQuizQuestions } from '../content/quizItems'

describe('malignant effusion assessment items', () => {
  it('has a meaningful number of questions', () => {
    expect(mpeQuizQuestions.length).toBeGreaterThanOrEqual(6)
  })

  it('every question has a valid answer index and distinct options', () => {
    for (const question of mpeQuizQuestions) {
