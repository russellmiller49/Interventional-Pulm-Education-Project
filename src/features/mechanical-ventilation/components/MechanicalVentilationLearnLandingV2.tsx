...
import { MechanicalVentilationNoviceRunway } from './MechanicalVentilationNoviceRunway'

/**
 * The Learn landing.
 *
 * `PathwayLanding` is shared and locked, and `notice` is the only node it renders between the hero
 * and the section list — which is exactly where a "before lesson 1" primer belongs. So the review
 * banner and the novice runway go through that one slot together, in that order: the boundary
 * statement first, then the runway, then the sections. Nothing about the pathway itself changes,
 * and the runway adds no section or activity id.
 */
          ...
          >
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                ...
                readiness. Numeric thresholds await this module&rsquo;s source reconciliation; use
                the reviewed practice cases for deeper clinical application.
              </p>
            </div>
          </aside>
          <MechanicalVentilationNoviceRunway />
        </div>
      }
    />
  )
