...
const commonSettingsSchema = {
  ...
  advanced: z.object({
    targetVtMl: z.number().min(20).max(2000),
    spontaneousPressureSupportCmH2O: z.number().min(0).max(100),
    spontaneousRampMs: z.number().min(0).max(2000),
    spontaneousCyclePercent: z.number().min(5).max(80),
    pHighCmH2O: z.number().min(1).max(100),
    pLowCmH2O: z.number().min(0).max(50),
    tHighSeconds: z.number().min(0.1).max(30),
    tLowSeconds: z.number().min(0.1).max(10),
    proportionalSupportPercent: z.number().min(5).max(95),
    minuteVolumePercent: z.number().min(25).max(350),
    targetSpO2LowPercent: z.number().min(85).max(98),
    targetPetCO2MmHg: z.number().min(25).max(80),
    automaticVentilationController: z.boolean(),
    automaticOxygenationController: z.boolean(),
    autoFlowEnabled: z.boolean(),
    intelliSyncEnabled: z.boolean(),
  }),
