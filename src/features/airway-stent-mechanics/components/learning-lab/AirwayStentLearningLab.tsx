...
import { getCasesForLesson, getPrimaryCaseForLesson } from '../../content/clinicalCaseRegistry'
...
export function AirwayStentLearningLab({
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      ...
      setActiveCaseId(getPrimaryCaseForLesson(lessonId)?.id ?? null)
  ...
  const recordInteraction = useCallback(
    (interaction: string, eventPayload: Record<string, unknown> = {}) => {
  ...
  const evidenceIds = useMemo(() => {
    ...
    return [...ids]
  }, [activeCases, activeLesson.evidenceRefs, activeLesson.id])
  ...
  function sendModuleCompletedIfReady(
    ...
    recordSiteModuleEvent({
      ...
      eventPayload: {
        ...
        bestPercent: nextProgress.assessment.bestPercent,
        attempts: nextProgress.assessment.attempts,
      },
    })
  }

  function completeLesson(lessonId: StentLessonId, caseId: string) {
    ...
    recordSiteModuleEvent({
      ...
      eventPayload: { caseId, interaction: 'clinical_case_completed' },
    })
  ...
  function completeOptionalEngineeringLab() {
    ...
    recordSiteModuleEvent({
      ...
      eventPayload: { interaction: 'optional_engineering_deep_dive_completed' },
    })
  }
          ...
          {activeLesson.id !== 'assessment' && selectedCase ? (
            <>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {activeCases.map((caseData) => (
                      <ClinicalCaseSummary
                        ...
                        caseData={caseData}
                        onSelect={() => setActiveCaseId(caseData.id)}
                      />
                    ))}
                  </div>
                </section>
              ...
              <ClinicalCaseFlow
                ...
                onComplete={(caseId) => completeLesson(activeLesson.id, caseId)}
                onDecisionCommitted={handleDecisionCommitted}
                onPhysicsLensOpen={handlePhysicsLensOpen}
              >
                {activeLesson.id === 'clinical-job' ? <MechanicalJobBuilder /> : null}
                {activeLesson.id === 'architecture-choice' ? <ArchitectureComparison /> : null}
                {activeLesson.id === 'fit-behavior' ? <FitPlanningChecklist /> : null}
                {activeLesson.id === 'complications-surveillance' ? (
                  <GranulationCase
                    ...
                    onSurveillancePlanCompleted={() =>
                      recordInteraction('surveillance_plan_completed', {
                        caseId: selectedCase.id,
                      })
                    }
                  />
