...
export function BaxterCrrtPractice({
  ...
  return (
    <BaxterCrrtModuleFrame locale={locale} activeHref={`${baxterCrrtNavBase}/practice`}>
      <header className={styles.sectionHero}>
        ...
        <p>
          The ten-case core path covers all six stations. Seven additional cases stay available for
          deeper practice; the unseen CRRT-16 capstone is excluded from every picker.
        </p>
      </header>
            ...
            {baxterCrrtCurriculum.map((unit) => (
              <optgroup key={unit.id} label={`${unit.station}. ${unit.title}`}>
                {unit.coreCaseIds.map((caseId) => {
                  const entry = getBaxterCrrtCaseCatalogEntry(caseId)
                  const complete = progress.completedPracticeCaseIds.includes(caseId.toLowerCase())
                  return (
