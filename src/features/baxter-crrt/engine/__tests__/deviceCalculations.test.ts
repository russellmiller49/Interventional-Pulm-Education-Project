...
describe('PrisMax calculation adapter boundary', () => {
  ...
  it('fails closed for a retired runtime profile', () => {
    expect(() => getCrrtDeviceCalculationAdapter('prismaflex-g5036003-6xx' as never)).toThrow(
      /Unsupported CRRT device calculation profile/i,
    )
  })
})
