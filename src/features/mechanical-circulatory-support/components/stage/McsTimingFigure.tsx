...
export function McsTimingFigure({
  ...
  const strip = timing && iabpStripView(state, timing, annotated ? 3 : 2)
    ...
    <figure className={styles.block} data-timing-figure={annotated ? 'reference' : 'independent'}>
      ...
      <details>
        ...
        <p>
          ...
          is {(timing.notchPhase * cycleMs).toFixed(0)} ms after cycle start. The balloon inflation
          band begins at {(timing.inflationPhase * cycleMs).toFixed(0)} ms and ends at{' '}
      ...
      <figcaption>
        Authored timing references, not detected clinical landmarks. Zero offset aligns this model’s
        event; it is not a universal manufacturer setting. The waveform cannot faithfully
        distinguish every early/late deflation contour.
      </figcaption>
    </figure>
  )
}
