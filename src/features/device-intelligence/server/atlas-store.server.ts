...
/**
 * The D1 atlas view of the governed catalog: the SAME statically imported generated JSON the
 ...
 * public-cohort predicate: `verification_grade = verified_source` AND `visibility_state =
 * prototype_visible` (decision D-07 as modified, 2026-08-08).
 *
 ...
 * excludes candidate-grade and hidden products by construction. `buildCatalogStore` already
 * drops role links, slot options, and related indexes for products absent from its input, so
 * there is no separate join to remember to filter.
 *
