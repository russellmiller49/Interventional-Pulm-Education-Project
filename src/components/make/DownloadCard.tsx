    ...
    <Card className="flex h-full flex-col justify-between border-border/70 bg-card/80">
        ...
        <div className="flex items-start gap-3">
          <div className="relative h-16 w-16 overflow-hidden rounded-xl border border-border/60 bg-muted">
            {resource.previewImage ? (
              <Image src={resource.previewImage} alt={resource.name} fill className="object-cover" sizes="64px" />
            ) : (
          ...
          <div className="space-y-1">
            <h3 className="text-lg font-semibold tracking-tight text-foreground">{resource.name}</h3>
      ...
      <CardContent className="space-y-4 text-sm text-muted-foreground">
        ...
        <ul className="space-y-1 text-xs text-muted-foreground/90">
          {resource.estimatedPrintTime ? <li>Estimated print/build time: {resource.estimatedPrintTime}</li> : null}
          {resource.estimatedMaterialCost ? <li>Estimated material cost: {resource.estimatedMaterialCost}</li> : null}
