      ...
      <section className="space-y-4">
        ...
        <ul className="grid gap-2 sm:grid-cols-2">
          {guide.objectives.map((objective) => (
            <li key={objective} className="flex items-start gap-2 rounded-2xl border border-border/60 bg-card/70 p-3 text-sm text-muted-foreground">
            ...
            <Card key={material.name} className="border-border/60 bg-card/80">
              <CardContent className="space-y-1 p-4 text-sm text-muted-foreground">
                <div className="flex items-center justify-between text-foreground">
                  ...
                  {material.cost ? <span className="text-xs text-muted-foreground">{material.cost}</span> : null}
                </div>
                <p className="text-xs uppercase tracking-[0.3em] text-muted-foreground/80">Quantity</p>
                <p>{material.quantity}</p>
                {material.notes ? <p className="text-xs text-muted-foreground/80">{material.notes}</p> : null}
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-4">
          {guide.assembly.map((step) => (
            <div key={step.title} className="grid gap-4 rounded-3xl border border-border/60 bg-card/70 p-4 sm:grid-cols-[0.8fr_1.2fr]">
              {step.image ? (
                <div className="relative h-40 overflow-hidden rounded-2xl border border-border/50 bg-muted">
                  <Image src={step.image} alt={step.title} fill className="object-cover" sizes="(min-width: 640px) 250px, 100vw" />
                </div>
              ) : (
      ...
      <section className="space-y-4">
        ...
        {guide.maintenance && guide.maintenance.length ? (
          <div className="rounded-3xl border border-dashed border-border/60 bg-muted/30 p-4 text-sm text-muted-foreground">
            <h3 className="text-xs font-semibold uppercase tracking-[0.4em] text-muted-foreground/80">Maintenance</h3>
