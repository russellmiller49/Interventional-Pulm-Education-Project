    ...
    <div className="space-y-6 rounded-3xl border border-border/70 bg-card/70 p-6">
      ...
      <div className="space-y-6">
        {questions.map((question, index) => {
          ...
          return (
            <div key={question.prompt} className="space-y-3 rounded-2xl border border-border/60 bg-background/60 p-4">
              <h4 className="text-sm font-semibold text-foreground">Q{index + 1}. {question.prompt}</h4>
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                >
                  {isRevealed ? 'Hide explanation' : choice === null ? 'Select an answer' : 'Check answer'}
                </Button>
                {choice !== null && !isRevealed ? (
                  <span className="text-xs text-muted-foreground/80">Select “Check answer” to reveal feedback.</span>
                ) : null}
              </div>
              {choice !== null && isRevealed ? (
                <div
