...
const markdownComponents: Components = {
  ...
  h3: (props) => <h3 className="text-base font-semibold tracking-tight text-foreground" {...props} />,
  ...
  thead: ({ children, ...props }) => (
    <thead className="bg-muted/40 text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground" {...props}>
      {children}
    </thead>
  ),
  ...
  th: (props) => <th className="border-b border-border/60 px-4 py-3 text-left text-muted-foreground" {...props} />,
  td: (props) => <td className="border-b border-border/40 px-4 py-3 align-top text-muted-foreground" {...props} />,
...
export function MarkdownContent({ content, className }: MarkdownContentProps) {
  ...
  return (
    <div className={cn('prose prose-sm prose-invert prose-headings:scroll-mt-28 max-w-none dark:prose-invert', className)}>
