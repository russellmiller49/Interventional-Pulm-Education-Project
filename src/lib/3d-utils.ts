...
async function loadModel(model: AnatomyModel): Promise<AnatomyAssetSuccess> {
  ...
  if (stlDownload) {
    ...
    const segments = model.segments.length ? model.segments : [{ id: 'default', name: 'Model', description: '', color: '#0ea5e9' }]
  ...
  const segments = model.segments.length ? model.segments : [{ id: 'placeholder', name: 'Segment', description: '', color: '#22d3ee' }]
...
export function applySegmentColors(group: Group, model: AnatomyModel) {
  ...
  group.traverse((child) => {
    if ((child as Mesh).isMesh) {
      const mesh = child as Mesh
      const segmentId = typeof mesh.userData.segmentId === 'string' ? mesh.userData.segmentId : segments[0].id
      ...
      segmentMeshes[segment.id] = segmentMeshes[segment.id] ? [...segmentMeshes[segment.id], mesh] : [mesh]
    }
  })
