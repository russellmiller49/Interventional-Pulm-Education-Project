...
import {
  getExactSlotReviewRuntimeContext,
...
export async function POST(request: Request) {
    ...
    return NextResponse.json(
      {
        error: {
          ...
          message: 'Choose a macro-free .xlsx clinician review workbook in a supported locale.',
        },
      },
      {
        status: 400,
  ...
  try {
    ...
    const runtimeContext = await getExactSlotReviewRuntimeContext()
    const preview = await importExactSlotReviewWorkbook(bytes, {
      applicationBaseUrl: new URL(request.url).origin,
      fileName,
      importedAt: new Date().toISOString(),
      locale,
      currentProposalArtifactSha256: runtimeContext.proposalArtifactSha256,
    })
    ...
    return NextResponse.json(
      {
        error: {
          code: 'WORKBOOK_VALIDATION_FAILED',
          message:
            'The workbook could not be validated. Confirm that it is an unencrypted, macro-free export from this Exact-slot review workflow.',
        },
      },
      {
        status: 422,
