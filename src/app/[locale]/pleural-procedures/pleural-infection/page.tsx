...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'pleuralInfection.overview' })
...
const base = pleuralInfectionNavBase

export default async function PleuralInfectionPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations('pleuralInfection')
  const nav = await getTranslations('navigation')

  const objectives = getInfectionObjectives(locale)
  ...
  return (
    <div className="space-y-10 py-16">
      ...
      />
      <PleuralInfectionNav activeHref={base} />
