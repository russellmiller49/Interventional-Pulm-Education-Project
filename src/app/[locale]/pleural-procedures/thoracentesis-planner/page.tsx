...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'thoracentesisPlanner.overview' })
...
const base = thoracentesisNavBase

export default async function ThoracentesisPlannerPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations('thoracentesisPlanner')
  const nav = await getTranslations('navigation')

  const objectives = getThoracentesisObjectives(locale)
  ...
  return (
    <div className="space-y-10 py-16">
      ...
      />
      <ThoracentesisNav activeHref={base} />
