...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'pleuralUltrasound.overview' })
...
const base = pleuralUltrasoundNavBase

export default async function PleuralUltrasoundPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations('pleuralUltrasound')
  const nav = await getTranslations('navigation')

  const objectives = getUltrasoundObjectives(locale)
  ...
  return (
    <div className="space-y-10 py-16">
      ...
      />
      <PleuralUltrasoundNav activeHref={base} />
