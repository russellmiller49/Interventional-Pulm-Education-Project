...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'malignantEffusion.overview' })
...
const base = malignantEffusionNavBase

export default async function MalignantEffusionPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations('malignantEffusion')
  const nav = await getTranslations('navigation')

  const objectives = getMpeObjectives(locale)
  ...
  return (
    <div className="space-y-10 py-16">
      ...
      />
      <MalignantEffusionNav activeHref={base} />
