...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'pleuroscopy.overview' })
...
const base = pleuroscopyNavBase

export default async function PleuroscopyPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations('pleuroscopy')
  const nav = await getTranslations('navigation')

  const objectives = getPleuroscopyObjectives(locale)
    ...
    <HandoffContent>
      {
        <div className="space-y-10 py-16">
          ...
          />
          <PleuroscopyNav activeHref={base} />
