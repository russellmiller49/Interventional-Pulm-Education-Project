...
import { pleuroscopyLessons } from '@/features/pleuroscopy/content/lessons'
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'pleuroscopy.references' })
...
function referencedIds(): string[] {
  ...
  for (const lesson of pleuroscopyLessons) {
...
export default async function PleuroscopyReferencesPage({ params }: PageProps) {
  const { locale } = await params
    ...
    <HandoffContent>
      {
        <div className="space-y-10 py-16">
          ...
          />
          <PleuroscopyNav activeHref="/pleural-procedures/pleuroscopy/references" />
