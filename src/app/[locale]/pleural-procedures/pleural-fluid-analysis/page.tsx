...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'pleuralFluidAnalysis.overview' })
...
const base = pleuralFluidAnalysisNavBase

export default async function PleuralFluidAnalysisPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations('pleuralFluidAnalysis')
  const nav = await getTranslations('navigation')

  const objectives = getFluidObjectives(locale)
  ...
  return (
    <div className="space-y-10 py-16">
      ...
      />
      <PleuralFluidAnalysisNav activeHref={base} />
