...
type ScopeProfileAdjustments = Record<ScopeViewProfileId, ScopeAdjustments>;
...
const SCOPE_VIEW_PROFILES: ScopeViewProfileId[] = ["flexible", "robotic"];
const DEFAULT_SCOPE_VIEW_PROFILE: ScopeViewProfileId = "flexible";
const SCOPE_PROFILE_QUERY_PARAM = "scopeProfile";
const HARDWARE_SCOPE_STORAGE_KEY = "bronchoedu.hardwareScope.v1";
const HARDWARE_STEER_MAX_PITCH_DEG = 55;
const HARDWARE_PUSH_CONFIRM_MM = 4;
const HARDWARE_MOVE_EPSILON_MM = 0.01;
...
export function App() {
  const [locale] = useState(localeFromWindow);
  const messages = useMemo(() => getMessages(locale), [locale]);
  const initialScopeViewProfile = useMemo(() => scopeViewProfileFromSearch(window.location.search), []);
  const scopeProfileForcedByUrlRef = useRef(initialScopeViewProfile != null);
  ...
  const [scopeViewProfile, setScopeViewProfile] = useState<ScopeViewProfileId>(initialScopeViewProfile ?? DEFAULT_SCOPE_VIEW_PROFILE);
  const [scopeAdjustmentProfiles, setScopeAdjustmentProfiles] = useState<ScopeProfileAdjustments>(emptyScopeProfileAdjustments);
  ...
  const scopeTracker = useScopeTrackerDrive(hardwareScopeEnabled, scopeHandlersRef);
  ...
  useEffect(() => {
    if (!hardwareScopeEnabled || !scopeTracker.connected) {
      setLiveSteer(null);
      setAimedEdgeId(null);
      scopeRollZeroRef.current = null;
      scopePushAccumRef.current = 0;
    }
  }, [hardwareScopeEnabled, scopeTracker.connected]);
  ...
  const noduleTargets = useMemo(() => (loadedCase ? noduleTargetsForCase(loadedCase.metadata, messages) : []), [loadedCase, messages]);
  ...
  const activeTargetLocations = useMemo(
    () => (activeTarget && indexes ? targetLocationsForTarget(activeTarget, indexes, activeNoduleAsset, messages) : []),
    [activeTarget, indexes, activeNoduleAsset, messages]
  );
    ...
    () =>
      beginnerTarget && indexes
        ? activeTarget?.id === beginnerTarget.id
          ...
          : targetLocationsForTarget(beginnerTarget, indexes, beginnerNoduleAsset, messages)
        : [],
    [activeTarget?.id, activeTargetLocations, beginnerTarget, indexes, beginnerNoduleAsset, messages]
  );
  ...
  if (!loadedCase || !indexes || !route || !focusRas || !airwayFrame || !airwayAxialFrame) {
    return (
      <main className="app app-centered">
        <div className="loading-panel">{messages.app.loadingCase}</div>
      </main>
    );
  }

  const takenPathEdgeIds = driveRouteComplete ? route.edgePath : committedPathEdgeIds;
  const scopeAdjustments = scopeAdjustmentProfiles[scopeViewProfile] ?? {};
  const scopeViewProfileLabel = messages.app.scopeProfiles[scopeViewProfile];
  ...
  const progressLabel = route.decisions.length ? `${Math.min(currentDecisionIndex + 1, route.decisions.length)} / ${route.decisions.length}` : messages.app.progressComplete;
  ...
  const scopeStatusLabel = debugFullPathActive
    ? debugFullPathComplete
      ? messages.app.status.fullPathComplete
      : driveRunning
        ? messages.app.status.fullPathPreview
        : messages.app.status.fullPathPaused
    : visibleDecision
      ? messages.app.status.decision(visibleDecision.index + 1)
      : driveRouteComplete
        ? messages.app.status.complete
        : driveRunning
          ? messages.app.status.driving
          : messages.app.status.paused;
  ...
  const selectScopeViewProfile = (profileId: ScopeViewProfileId) => {
    setScopeViewProfile(profileId);
    updateScopeProfileQuery(profileId);
  };
  ...
  // Latest-ref handlers for the hardware scope tracker: reassigned every render so the
  // rAF poll loop (useScopeTrackerDrive) always sees fresh state and actions.
  scopeHandlersRef.current = {
    onFrame: (frame, deltas) => {
      if (scopeRollZeroRef.current === null || frame.pressed.calibrate) {
        scopeRollZeroRef.current = frame.rollContinuousRad;
      }
      const pitchDeg = frame.flexion * HARDWARE_STEER_MAX_PITCH_DEG;
      const rollDeg = ((frame.rollContinuousRad - scopeRollZeroRef.current) * 180) / Math.PI;
      setLiveSteer((current) =>
        current && Math.abs(current.pitchDeg - pitchDeg) < 0.05 && Math.abs(current.rollDeg - rollDeg) < 0.05
          ? current
          : { pitchDeg, rollDeg }
      );

      if (Math.abs(deltas.dDepthMm) > HARDWARE_MOVE_EPSILON_MM) {
        if (driveRunning) {
          setDriveRunning(false);
        }
        setDriveDistanceMm((current) => clamp(current + deltas.dDepthMm, 0, driveTargetDistanceMm));
      }

      // Push-to-commit: with a branch selected, deliberately advancing the physical
      // scope confirms the choice (same action as the "Drive on" button).
      if (visibleDecision && selectedEdgeId != null && deltas.dDepthMm > 0) {
        scopePushAccumRef.current += deltas.dDepthMm;
        if (scopePushAccumRef.current >= HARDWARE_PUSH_CONFIRM_MM) {
          scopePushAccumRef.current = 0;
          continueDrive();
        }
      } else if (deltas.dDepthMm < 0) {
        scopePushAccumRef.current = 0;
      }
      ...
      if (frame.pressed.a) {
        if (visibleDecision) {
          if (selectedEdgeId == null) {
            if (aimedEdgeId != null && visibleDecision.options.some((option) => option.edgeId === aimedEdgeId)) {
              chooseOption(aimedEdgeId);
            }
          } else {
            continueDrive();
          }
        } else if (driveRouteComplete) {
          resetPractice();
        } else {
          toggleDrive();
        }
      ...
      if (frame.pressed.b) {
        if (visibleDecision) {
          if (selectedEdgeId != null && !testMode) {
            setSelectedEdgeId(null);
          }
        } else {
          toggleDrive();
      ...
      if ((frame.pressed.c || frame.pressed.d) && visibleDecision && !testMode) {
        const options = visibleDecision.options;
        if (options.length > 0) {
          const direction = frame.pressed.d ? 1 : -1;
          const currentIndex = options.findIndex((option) => option.edgeId === selectedEdgeId);
          const nextIndex =
            currentIndex < 0
              ? direction > 0
                ? 0
          ...
          chooseOption(options[nextIndex].edgeId);
        }
      }
    },
    onDisconnect: () => {
  ...
  const moveTarget = (delta: number) => {
    ...
    setTargetPlaced(true);
    setTargetPlacementStatus(messages.app.targetPlacement.target(nextIndex + 1));
  };
  ...
  const toggleCentralAirwayReview = () => {
    ...
    jumpToBeginnerTargetIndex(firstFinding.targetIndex, messages.app.targetPlacement.centralCheck(firstFinding.targetNumber));
  };
  ...
  const stepCentralAirwayFinding = (delta: number) => {
    ...
    if (nextFinding) {
      jumpToBeginnerTargetIndex(nextFinding.targetIndex, messages.app.targetPlacement.centralCheck(nextFinding.targetNumber));
    }
  };
  ...
  const snapTargetToRas = (ras: Vec3) => {
    ...
    setTargetLocationIndex(nextIndex);
    setTargetPlaced(true);
    setTargetPlacementStatus(messages.app.targetPlacement.snappedToTarget(nextIndex + 1));
  };
  ...
  const updateCurrentScopeAdjustment = (updater: (current: ScopeAdjustment) => ScopeAdjustment) => {
    ...
    setScopeAdjustmentProfiles((current) => {
      return {
        ...current,
        [scopeViewProfile]: {
          ...current[scopeViewProfile],
          [scopeAdjustmentKey]: nextAdjustment
        }
      };
    });
    queueScopeSourceSave(scopeViewProfile, scopeViewProfileLabel, scopeAdjustmentKey, nextAdjustment);
  };
  ...
  const resetCurrentScopeAdjustment = () => {
    ...
    setScopeAdjustmentProfiles((current) => {
      const nextProfileAdjustments = { ...current[scopeViewProfile] };
      delete nextProfileAdjustments[scopeAdjustmentKey];
      return {
        ...current,
        [scopeViewProfile]: nextProfileAdjustments
      };
    });
    queueScopeSourceSave(scopeViewProfile, scopeViewProfileLabel, scopeAdjustmentKey, null);
  };

  const queueScopeSourceSave = (profileId: ScopeViewProfileId, profileLabel: string, nodeKey: string, adjustment: ScopeAdjustment | null) => {
    if (sourceSaveTimerRef.current != null) {
      window.clearTimeout(sourceSaveTimerRef.current);
    }
    setCalibrationStatus(
      adjustment ? messages.app.calibration.savingNode(nodeKey, profileLabel) : messages.app.calibration.removingNode(nodeKey, profileLabel)
    );
    sourceSaveTimerRef.current = window.setTimeout(() => {
      sourceSaveTimerRef.current = null;
      void saveScopeAdjustmentToSource(loadedCase.metadata.caseId, profileId, nodeKey, adjustment, messages.app.calibration.sourceSaveFailed)
        .then(() => {
          setCalibrationStatus(
            adjustment
              ? messages.app.calibration.savedNode(nodeKey, profileLabel, SCOPE_CALIBRATION_SOURCE_PATH)
              : messages.app.calibration.removedNode(nodeKey, profileLabel, SCOPE_CALIBRATION_SOURCE_PATH)
          );
        })
        .catch((saveError: unknown) => {
          const message = saveError instanceof Error ? saveError.message : String(saveError);
          setCalibrationStatus(messages.app.calibration.localOnly(message));
        });
    }, 250);
  };
    ...
    visibleDecision ? (
      <>
          ...
          {visibleDecision.options.map((option) => {
            ...
            return (
              <button
                ...
                aria-label={messages.app.branchSelectAria(option.label)}
              >
        ...
        <Feedback
          ...
          selectedOptionCorrect={selectedOption?.isCorrect ?? null}
          remainingPathCount={selectedOption?.correctTerminalNodeIds?.length ?? 0}
          testMode={testMode}
          messages={messages.app.feedback}
        />
        <button className="primary-action" disabled={selectedEdgeId == null} onClick={continueDrive}>
          {messages.app.actions.driveOn}
        </button>
      </>
    ) : driveRouteComplete ? (
      <>
        {testMode && <TestScoreSummary correctCount={testCorrectCount} incorrectCount={testIncorrectCount} answeredCount={testAnsweredCount} scorePercent={testScorePercent} scoreLabel={testScoreLabel} messages={messages.app.score} />}
        <RouteCompleteCelebration messages={messages.app.complete} />
        <button className="primary-action" onClick={resetPractice}>
          {testMode ? messages.app.actions.restartTest : messages.app.actions.restartRoute}
        </button>
      </>
    ) : (
      <>
        ...
        <div className="scope-footer-primary">
          <div className="scope-footer-transport">
            <button className="icon-action" onClick={() => nudgeDrive(-8)} aria-label={messages.app.actions.moveScopeBackward}>
              {"<"}
            </button>
            <button className="icon-action" onClick={() => nudgeDrive(8)} aria-label={messages.app.actions.moveScopeForward} disabled={atDecisionStop}>
              {">"}
            </button>
          </div>
          <button className="primary-action" onClick={toggleDrive} disabled={atDecisionStop}>
            {driveRunning ? messages.app.actions.pause : messages.app.actions.driveToBranch}
          </button>
        </div>
      </>
    )
    ...
    <main className="app">
      <header className="topbar">
        <div className="brand-block">
          <strong>{messages.app.brand}</strong>
          <span>{loadedCase.metadata.caseId}</span>
        </div>
        <div className="wizard-stepper" role="group" aria-label={messages.app.wizard.trainerStepsAria}>
          {([
            { n: 1 as const, label: messages.app.wizard.placeTarget },
            { n: 2 as const, label: messages.app.wizard.practice },
            { n: 3 as const, label: messages.app.wizard.test }
          ]).map((step) => {
        ...
        <div className="segmented scope-profile-selector" role="group" aria-label={messages.app.scopeProfiles.ariaLabel}>
          {SCOPE_VIEW_PROFILES.map((profileId) => (
            <button key={profileId} className={scopeViewProfile === profileId ? "active" : ""} onClick={() => selectScopeViewProfile(profileId)}>
              {messages.app.scopeProfiles[profileId]}
            </button>
          ))}
        </div>
        <div className="wizard-nav">
          <button className="secondary-action" onClick={wizardBack} disabled={wizardStep === 1}>
            {messages.app.actions.back}
          </button>
          <button
            className="primary-action"
            onClick={wizardNext}
            disabled={wizardStep === 3 || (wizardStep === 1 && !canAdvanceToPractice)}
            aria-label={wizardStep === 1 && !canAdvanceToPractice ? messages.app.wizard.placeTargetFirst : undefined}
          >
            {messages.app.actions.next}
          </button>
        </div>
        {!testMode && (
          <label className="toggle">
            <input type="checkbox" checked={showRoute} onChange={(event) => setShowRoute(event.target.checked)} />
            <span>{messages.app.toggles.centerline}</span>
          </label>
        )}
        {setupMode && ENABLE_AUTHORING_TOOLS && (
          <label className={`toggle ${hasCandidateLabels ? "" : "toggle-disabled"}`}>
            ...
            />
            <span>{messages.app.toggles.candidates}</span>
          </label>
        )}
        {!testMode && (
          <label className="toggle">
            ...
            />
            <span>{messages.app.toggles.airwayCt}</span>
          </label>
        )}
        {setupMode && ENABLE_SCOPE_DEBUG && (
          <label className="toggle">
            ...
            <span>{messages.app.toggles.scopeDebug}</span>
          </label>
        )}
        {!testMode && (
          <label className="toggle">
            <input type="checkbox" checked={showScopeCompass} onChange={(event) => setShowScopeCompass(event.target.checked)} />
            <span>{messages.app.toggles.compass}</span>
          </label>
        )}
        {!testMode && (
          <label className="toggle">
            <input type="checkbox" checked={showScopeTumor} onChange={(event) => setShowScopeTumor(event.target.checked)} />
            <span>{messages.app.toggles.scopeTumor}</span>
          </label>
        )}
        {!testMode && (
          <label className="toggle">
            <input type="checkbox" checked={showScopeTrace} onChange={(event) => setShowScopeTrace(event.target.checked)} />
            <span>{messages.app.toggles.scopeTrace}</span>
          </label>
        )}
        <label className="toggle">
          <input type="checkbox" checked={hardwareScopeEnabled} onChange={(event) => setHardwareScopeEnabled(event.target.checked)} />
          <span>{messages.app.hardware.toggle}</span>
        </label>
        {hardwareScopeEnabled && (
          <span
            className={`hardware-status ${scopeTracker.connected ? "hardware-status-connected" : ""} ${scopeTracker.lowQuality ? "hardware-status-warning" : ""}`}
            title={scopeTracker.lowQuality ? messages.app.hardware.lowQuality : (scopeTracker.deviceId ?? undefined)}
          >
            {scopeTracker.connected
              ? scopeTracker.lowQuality
                ? messages.app.hardware.lowQualityShort
                : messages.app.hardware.connected
              : messages.app.hardware.disconnected}
          </span>
        )}
        <div className="case-status">
          <span>{setupMode ? messages.app.caseStatus.mode : testMode && driveRouteComplete ? messages.app.caseStatus.score : messages.app.caseStatus.decision}</span>
          <strong>{setupMode ? messages.app.caseStatus.setup : testMode && driveRouteComplete ? `${testScorePercent}%` : progressLabel}</strong>
        </div>
      </header>
      ...
      <aside className="trainer-panel">
        <div className="panel-section guide-section">
          <span className="section-label">
            {messages.app.wizard.stepOfThree(wizardStep, wizardStep === 1 ? messages.app.wizard.placeTarget : wizardStep === 2 ? messages.app.wizard.practice : messages.app.wizard.test)}
          </span>
          {setupMode ? (
            <ol className="instruction-list">
              {messages.app.instructions.setup.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ol>
          ) : testMode ? (
            <ol className="instruction-list">
              {messages.app.instructions.test.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ol>
          ) : (
            <ol className="instruction-list">
              {messages.app.instructions.practice.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ol>
          )}
        ...
        <div className="panel-section">
          <span className="section-label">{messages.app.target.section}</span>
          ...
          <h1>{activeTarget?.label ?? messages.app.target.noduleTarget}</h1>
          {testMode ? (
            <p>{messages.app.target.testHelp}</p>
          ) : (
            <p>{messages.app.target.status(targetPlaced, targetOrdinal, targetCount, pathCount, pathsNarrowed)}</p>
          )}
          {setupMode ? (
            <>
              <div className="nodule-picker">
                <NoduleThumbnail asset={activeNoduleAsset} label={activeTarget?.label ?? messages.caseLabels.noduleFallback} previewAriaLabel={messages.app.target.nodulePreviewAria} onDragStart={startNoduleDrag} />
                <button className="secondary-action surprise-action" onClick={surpriseTarget} disabled={activeTargetLocations.length < 2}>
                  {messages.app.target.surpriseMe}
                </button>
              </div>
              <p className="placement-help">{messages.app.target.placementHelp}</p>
              {ENABLE_AUTHORING_TOOLS && beginnerTarget && (
                <div className={`central-review-panel ${showCentralAirwayReview ? "central-review-panel-active" : ""}`}>
                  <button className="secondary-action wide central-review-trigger" onClick={toggleCentralAirwayReview} disabled={!centralAirwayFindings.length}>
                    {showCentralAirwayReview ? messages.app.target.hideCentralCheck : messages.app.target.centralAirwayCheck}
                  </button>
                  <div className="decision-meta central-review-summary">
                    <span>{messages.app.target.flaggedTargets(centralAirwayFindings.length)}</span>
                    <span>{activeCentralAirwayFindingIndex >= 0 ? `${activeCentralAirwayFindingIndex + 1}/${centralAirwayFindings.length}` : messages.app.target.reviewQueue}</span>
                  </div>
                    ...
                    <>
                      <div className="inline-actions central-review-actions">
                        <button className="secondary-action" onClick={() => stepCentralAirwayFinding(-1)}>
                          {messages.app.target.prevFlagged}
                        </button>
                        <button className="secondary-action" onClick={() => stepCentralAirwayFinding(1)}>
                          {messages.app.target.nextFlagged}
                        </button>
                      </div>
                      <div className="central-review-list" aria-label={messages.app.target.beginnerTargetsNearCentralAirways}>
                        {centralAirwayFindings.map((finding) => (
                          <button
                            ...
                            onClick={() => jumpToBeginnerTargetIndex(finding.targetIndex, messages.app.targetPlacement.centralCheck(finding.targetNumber))}
                          >
                            <strong>{messages.app.target.targetNumber(finding.targetNumber)}</strong>
                            <span>
              ...
              <div className="path-list" aria-label={messages.app.target.acceptedPathsForTarget}>
                {activePathTerminalIds.map((nodeId, index) => (
                  <button key={nodeId} className={`path-pill ${nodeId === selectedEndpointId ? "path-pill-active" : ""}`} onClick={() => selectPathTerminal(nodeId)}>
                    {messages.app.target.pathNumber(index + 1)}
                  </button>
                ))}
              </div>
              <div className="inline-actions">
                <button className="secondary-action" onClick={() => moveTarget(-1)} disabled={activeTargetLocations.length < 2}>
                  {messages.app.target.prevTarget}
                </button>
                <button className="secondary-action" onClick={() => moveTarget(1)} disabled={activeTargetLocations.length < 2}>
                  {messages.app.target.nextTarget}
                </button>
              </div>
              <button className="secondary-action wide" onClick={resetTarget}>
                {messages.app.target.resetTarget}
              </button>
            </>
          ) : (
            <button className="secondary-action wide" onClick={() => enterMode("setup")}>
              {messages.app.target.adjustTarget}
            </button>
          )}
        </div>

        <div className="panel-section">
          <span className="section-label">{messages.app.ctViews.section}</span>
          <div className="decision-meta">
            <span>{effectiveCtViewMode === "airway" ? messages.app.ctViews.airwayAligned : messages.app.ctViews.standardPlanes}</span>
            <span>{messages.app.ctViews.scrollSlices}</span>
          </div>
          <button className="secondary-action wide" onClick={resetSlices}>
            {messages.app.ctViews.recenterSlices}
          </button>
          <div className="zoom-row">
            <button className="icon-action" onClick={() => nudgeZoom(-0.25)} aria-label={messages.app.ctViews.zoomOutAria}>
              -
            </button>
            <label className="range-control">
              <span>{messages.app.ctViews.zoomLabel(ctZoom.toFixed(2))}</span>
              <input type="range" min="1" max="4" step="0.25" value={ctZoom} onChange={(event) => setZoom(Number(event.target.value))} />
            </label>
            <button className="icon-action" onClick={() => nudgeZoom(0.25)} aria-label={messages.app.ctViews.zoomInAria}>
              +
            </button>
          </div>
        </div>
        ...
        {setupDebugEnabled && (
          <div className="panel-section debug-section">
            <span className="section-label">{messages.app.debug.section}</span>
            <div className="decision-meta">
              <span>{visibleDecision ? messages.app.debug.nodeLabel(visibleDecision.nodeId) : messages.app.debug.noActiveBranch}</span>
              <span>{messages.app.debug.profileSource(scopeViewProfileLabel, SCOPE_CALIBRATION_SOURCE_PATH)}</span>
            </div>
            <div className="debug-preview-control">
              <div className="decision-meta">
                <span>{debugFullPathActive ? (debugFullPathComplete ? messages.app.status.fullPathComplete : messages.app.status.fullPathPreview) : messages.app.status.fullPathPreview}</span>
                <span>
                  {Math.round(driveDistanceMm)} / {Math.round(route.totalLengthMm)} mm
                </span>
              </div>
              <div className="drive-controls">
                <button className="icon-action" onClick={() => nudgeDebugFullPath(-8)} disabled={!debugFullPathActive || driveDistanceMm <= 0} aria-label={messages.app.debug.moveFullPathPreviewBackward}>
                  {"<"}
                </button>
                <button className="secondary-action" onClick={toggleDebugFullPathDrive}>
                  {!debugFullPathActive ? messages.app.debug.driveFullPath : driveRunning ? messages.app.actions.pause : debugFullPathComplete ? messages.app.debug.replay : messages.app.debug.resume}
                </button>
                <button
                  className="icon-action"
                  onClick={() => nudgeDebugFullPath(8)}
                  disabled={!debugFullPathActive || driveDistanceMm >= route.totalLengthMm - 0.75}
                  aria-label={messages.app.debug.moveFullPathPreviewForward}
                >
                  {">"}
                </button>
              </div>
              <label className="range-control drive-range">
                <span>{messages.app.debug.previewPosition(Math.round(driveDistanceMm))}</span>
                <input
                  type="range"
                  min="0"
              ...
              {debugFullPathActive && (
                <button className="secondary-action wide" onClick={exitDebugFullPath}>
                  {messages.app.debug.returnToBranchView}
                </button>
              )}
            </div>
            {!debugFullPathActive && (
              <>
                <div className="inline-actions">
                  <button className="secondary-action" onClick={() => debugMoveDecision(-1)}>
                    {messages.app.debug.prevView}
                  </button>
                  <button className="secondary-action" onClick={() => debugMoveDecision(1)}>
                    {messages.app.debug.nextView}
                  </button>
                </div>
                <p className="debug-help">{messages.app.debug.help}</p>
                <DebugSlider
                  label={messages.app.debug.sliders.back}
                  value={visibleScopeAdjustment.cameraBackMm}
                  min={scopeCameraBackMinMm}
                  max={scopeCameraBackMaxMm}
                  step={1}
                  suffix={messages.app.debug.sliders.millimeters}
                  onChange={(value) => updateCurrentScopeAdjustment((current) => ({ ...current, cameraBackMm: value }))}
                />
                <DebugSlider
                  label={messages.app.debug.sliders.aim}
                ...
                />
                <DebugSlider
                  label={messages.app.debug.sliders.yaw}
                  value={scopeAdjustment.yawDeg}
                  min={-80}
                  max={80}
                  step={1}
                  suffix={messages.app.debug.sliders.degrees}
                  onChange={(value) => updateCurrentScopeAdjustment((current) => ({ ...current, yawDeg: value }))}
                />
                <DebugSlider
                  label={messages.app.debug.sliders.pitch}
                ...
                />
                <DebugSlider
                  label={messages.app.debug.sliders.roll}
                ...
                />
                <DebugSlider
                  label={messages.app.debug.sliders.fov}
                ...
                <button className="secondary-action wide" onClick={resetCurrentScopeAdjustment}>
                  {messages.app.debug.resetScopeView}
                </button>
              </>
            )}
        ...
        {!testMode && (
          <div className="panel-section compact-stats">
            <div>
              <span>{messages.app.stats.routeEdges}</span>
              <strong>{route.edgePath.length}</strong>
            </div>
            <div>
              <span>{messages.app.stats.terminals}</span>
      ...
      <section className={`workspace ${testMode ? "workspace-test" : ""}`}>
        ...
        <div className={`right-stack ${testMode ? "right-stack-test" : ""}`}>
          <BronchoscopeView
            ...
            liveSteer={liveSteer}
            onAimOption={liveSteer ? setAimedEdgeId : undefined}
            footer={scopeFooter}
...
function noduleTargetsForCase(webCase: WebCase, messages: Messages): NoduleTarget[] {
  ...
  return webCase.noduleAsset
    ? [
        {
          id: "advanced",
          label: messages.caseLabels.noduleTargets.advanced,
...
function targetLocationsForTarget(target: NoduleTarget, indexes: CaseIndexes, noduleAsset: LoadedNoduleAsset | null, messages: Messages): NoduleTargetLocation[] {
  ...
  const generatedLocations = (anchorNodeIds.length ? anchorNodeIds : [target.initialTerminalNodeId]).map((anchorNodeId, index) => {
    ...
    if (endpointOverride != null && indexes.nodesById.has(endpointOverride)) {
      return {
        ...
        label: messages.caseLabels.targetLocation(index + 1),
        targetRas,
...
function NoduleThumbnail({
  ...
  label: string;
  previewAriaLabel: (label: string) => string;
  ...
  return (
    <div className="nodule-thumbnail" draggable onDragStart={onDragStart} role="img" aria-label={previewAriaLabel(label)}>
...
function RouteCompleteCelebration({ messages }: { messages: Messages["app"]["complete"] }) {
  return (
    <div className="done-state celebration-state" role="status" aria-live="polite">
      ...
      <strong>{messages.lesionReached}</strong>
      <span>{messages.routeComplete}</span>
    </div>
  );
}
...
function TestScoreSummary({
  ...
  scorePercent: number;
  scoreLabel: string;
  messages: Messages["app"]["score"];
}) {
  return (
    <div className="test-score-summary" role="status" aria-live="polite">
      <span className="section-label">{messages.section}</span>
      <strong>{scorePercent}%</strong>
      <span>{messages.line(scoreLabel, answeredCount)}</span>
...
function emptyScopeProfileAdjustments(): ScopeProfileAdjustments {
  return {
    flexible: {},
    robotic: {}
  };
...
function parseScopeCalibrationProfiles(value: unknown): { defaultProfile: ScopeViewProfileId; profiles: ScopeProfileAdjustments } {
  const legacyAdjustments = parseScopeCalibration(value);
  const profilePayloads = isRecord(value) && isRecord(value.profiles) ? value.profiles : {};
  const profiles = emptyScopeProfileAdjustments();
  for (const profileId of SCOPE_VIEW_PROFILES) {
    const rawProfile = profilePayloads[profileId];
    profiles[profileId] = isRecord(rawProfile) ? parseScopeCalibration(rawProfile) : legacyAdjustments;
  }
  const defaultProfile = isRecord(value) ? parseScopeViewProfileId(value.defaultProfile) ?? DEFAULT_SCOPE_VIEW_PROFILE : DEFAULT_SCOPE_VIEW_PROFILE;
...
function scopeViewProfileFromSearch(search: string): ScopeViewProfileId | null {
  return parseScopeViewProfileId(new URLSearchParams(search).get(SCOPE_PROFILE_QUERY_PARAM));
}

function updateScopeProfileQuery(profileId: ScopeViewProfileId) {
  const url = new URL(window.location.href);
  if (profileId === DEFAULT_SCOPE_VIEW_PROFILE) {
    url.searchParams.delete(SCOPE_PROFILE_QUERY_PARAM);
  } else {
    url.searchParams.set(SCOPE_PROFILE_QUERY_PARAM, profileId);
  }
  window.history.replaceState({}, "", url);
}

function parseScopeViewProfileId(value: unknown): ScopeViewProfileId | null {
...
async function saveScopeAdjustmentToSource(
  caseId: string,
  profileId: ScopeViewProfileId,
  nodeKey: string,
  adjustment: ScopeAdjustment | null,
  sourceSaveFailed: (status: number) => string
) {
  ...
  if (!response.ok) {
    ...
    throw new Error(message || sourceSaveFailed(response.status));
  }
}
...
function buildCandidateOverlays(currentDecision: Decision, edgesById: Map<number, AirwayEdge>): CandidateOverlay[] {
  ...
  return currentDecision.options.flatMap((option, index) => {
    ...
    return [
      {
        edge,
        label: candidate.displayLabel ?? candidate.candidateLabel,
...
function Feedback({
  ...
  if (selectedEdgeId == null) {
    return <div className="feedback neutral">{messages.chooseBranch}</div>;
  }
  if (selectedOptionCorrect) {
    if (testMode) {
      return <div className="feedback correct">{messages.correctFirstAttempt}</div>;
    }
    const pathCount = Math.max(remainingPathCount, 1);
    return <div className="feedback correct">{messages.correctPathsRemain(pathCount)}</div>;
  }
  if (testMode) {
    return <div className="feedback wrong">{messages.wrongFirstAttempt}</div>;
  }
  return <div className="feedback wrong">{messages.wrongShowCorrect}</div>;
}
