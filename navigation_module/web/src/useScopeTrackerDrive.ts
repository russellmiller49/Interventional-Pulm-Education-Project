...
/**
 * Polls the physical scope tracker (USB HID gamepad, see src/scope-input/) once per
 * animation frame while enabled. Handlers are read through a ref that the caller
 * reassigns every render, so they always close over fresh state.
 */
export function useScopeTrackerDrive(
  enabled: boolean,
