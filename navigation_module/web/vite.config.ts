    ...
    configureServer(server) {
      server.middlewares.use(endpoint, async (request, response) => {
        ...
        try {
          ...
          response.end(JSON.stringify({ ok: true, path: "/cases/default/scope_calibration.json", profileId }));
        } catch (error) {
...
function normalizeScopeCalibrationPayload(parsed: Record<string, unknown>) {
  ...
  const defaultProfile = parseScopeViewProfileId(parsed.defaultProfile) ?? DEFAULT_SCOPE_VIEW_PROFILE;
  const payload = {
    ...parsed,
    schema: SCOPE_CALIBRATION_SCHEMA,
    defaultProfile,
...
function normalizeScopeCalibrationProfiles(rawProfiles: unknown, legacyAdjustments: Record<string, unknown>) {
  ...
  for (const profileId of SCOPE_VIEW_PROFILE_IDS) {
    ...
    profiles[profileId] = {
      ...profiles[profileId],
      ...rawProfile,
      label: typeof rawProfile.label === "string" ? rawProfile.label : SCOPE_VIEW_PROFILE_LABELS[profileId],
      adjustments: isRecord(rawProfile.adjustments) ? rawProfile.adjustments : cloneRecord(legacyAdjustments)
    };
  }
  return profiles;
}
