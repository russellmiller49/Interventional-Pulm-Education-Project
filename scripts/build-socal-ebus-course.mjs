...
const defaultAuthCallbackUrl = 'https://interventionalpulm.org/auth/callback?app=socal-ebus-course'

loadEnvConfig(projectRoot)

const embeddedEnv = {
  ...process.env,
  VITE_PUBLIC_SITE_URL: process.env.VITE_PUBLIC_SITE_URL ?? process.env.NEXT_PUBLIC_SITE_URL ?? '',
  ...
  VITE_SUPABASE_AUTH_CALLBACK_URL:
    process.env.VITE_SUPABASE_AUTH_CALLBACK_URL ??
    process.env.NEXT_PUBLIC_SUPABASE_AUTH_CALLBACK_URL ??
...
if (!embeddedEnv.VITE_SUPABASE_URL || !embeddedEnv.VITE_SUPABASE_ANON_KEY) {
  console.log(
    'Hint: put NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in .env.local or export VITE_SUPABASE_* before building.',
  )
}
...
console.log(`SoCal EBUS course built at ${destinationDir}`)
