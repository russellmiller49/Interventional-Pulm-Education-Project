...
def main():
    ...
    x = x*meta['spacingMm'][0] + meta['originMm'][0] - 85
    y = y*meta['spacingMm'][1] + meta['originMm'][1] + 20
    z = z*meta['spacingMm'][2] + meta['originMm'][2] + 30
    target = np.clip(9.5-np.sqrt(x*x+y*y+z*z), 0, 1)
    density = density*(1-target) + 1.06*target
    ...
    (out/'dts.json').write_text(json.dumps({
        ...
        'targetCenterMm': [85, -20, -30], 'toolPlaneRelativeMm': -18,
