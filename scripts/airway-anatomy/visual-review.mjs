...
await page.waitForFunction(
  () =>
    Array.from(document.querySelectorAll('button')).some(
      (b) => b.textContent === 'Realistic navigation' && !b.disabled,
    ),
  null,
  { timeout: 60000 },
)
for (const reference of ['RUL', 'RML', 'LUL']) {
  ...
  await page.screenshot({
    path: path.join(output, `${reference.toLowerCase()}.png`),
    fullPage: true,
  })
  await page
    .getByText('Virtual bronchoscopy', { exact: true })
    .locator('..')
    .screenshot({ path: path.join(output, `${reference.toLowerCase()}-optical.png`) })
}
...
await writeFile(
  ...
  JSON.stringify({ url: base, viewport: { width: 1600, height: 1100 }, errors }, null, 2),
)
console.log(JSON.stringify({ output, errors }))
await context.close()
await page.video()?.saveAs(path.join(output, 'synchronized-review.webm'))
await browser.close()
