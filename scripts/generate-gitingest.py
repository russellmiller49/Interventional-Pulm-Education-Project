...
Adapted for Next.js/TypeScript repository.
"""
...
EXCLUDED_DIRS = {
    ...
    ".vscode",
    ".idea",
    "in-development",  # Excluded from builds
}
...
EXCLUDED_FILE_EXTENSIONS = {
    ...
    ".swp",
    ".swo",
    # 3D model formats (except .glb which may be needed)
    ".obj",
    ".fbx",
    ".dae",
    ".3ds",
...
def iter_detail_candidate_files(
    ...
    # Stable ordering: prioritize scripts and config files, then smaller files.
    def sort_key(p: Path) -> tuple[int, int, int, str]:
        rel = str(p.relative_to(repo_root)).replace("\\", "/")
        in_scripts = 0 if rel.startswith("scripts/") else 1
        is_config = 0 if any(rel.endswith(ext) for ext in [".config.ts", ".config.js", ".config.mjs", "tsconfig.json", "package.json"]) else 1
        try:
            size = p.stat().st_size
        except Exception:
            size = 10**9
        return (in_scripts, is_config, size, rel.lower())
...
def generate_gitingest_details(
    ...
    def should_inline(rel_path: str, suffix: str) -> bool:
        ...
        if inline_mode == "all":
            return True
        # curated
        if rel_path.startswith("scripts/") and suffix in {".py", ".js", ".ts", ".md"}:
            return True
        if rel_path.startswith("src/") and suffix in {".ts", ".tsx", ".js", ".jsx"}:
            return True
        if rel_path.startswith("content/") and suffix in {".md", ".mdx"}:
            return True
        if any(rel_path.endswith(ext) for ext in [".config.ts", ".config.js", ".config.mjs", "tsconfig.json", "package.json"]):
            return True
        return False
    ...
    for p in candidates:
        ...
        if should_inline(rel, suffix):
            ...
            if suffix in {".js", ".ts", ".jsx", ".tsx"} and looks_minified_text(text):
    ...
    content_parts = [
        "# Interventional Pulmonology Website — gitingest (details)",
        "",
...
def generate_gitingest(repo_root: Path, output_path: Path) -> None:
    ...
    content_parts = [
        "# Interventional Pulmonology Website — gitingest (curated)",
        "",
        f"Generated: `{timestamp}`",
        f"Git: `{branch}` @ `{commit}`",
        "",
        ...
        "- Intended for LLM/context ingestion; excludes large artifacts (build outputs, node_modules, caches, 3D models).",
        "",
