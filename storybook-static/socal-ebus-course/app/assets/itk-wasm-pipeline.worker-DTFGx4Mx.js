 ...
 */ const de = Symbol('Comlink.proxy'),
  $e = Symbol('Comlink.endpoint'),
  At = Symbol('Comlink.releaseProxy'),
  mA = Symbol('Comlink.finalizer'),
  QA = Symbol('Comlink.thrown'),
  ye = (A) => (typeof A == 'object' && A !== null) || typeof A == 'function',
  et = {
    canHandle: (A) => ye(A) && A[de],
    serialize(A) {
  ...
  tt = {
    canHandle: (A) => ye(A) && QA in A,
    serialize({ value: A }) {
      let e
      return (
        A instanceof Error
          ? (e = { isError: !0, value: { message: A.message, name: A.name, stack: A.stack } })
          : (e = { isError: !1, value: A }),
        [e, []]
      )
    },
...
function It(A, e) {
  for (const t of A) if (e === t || t === '*' || (t instanceof RegExp && t.test(e))) return !0
  return !1
}
function OA(A, e = globalThis, t = ['*']) {
  ;(e.addEventListener('message', function I(g) {
    if (!g || !g.data) return
    if (!It(t, g.origin)) {
      console.warn(`Invalid origin '${g.origin}' for comlink proxy`)
      return
    }
    const { id: n, type: r, path: o } = Object.assign({ path: [] }, g.data),
      c = (g.data.argumentList || []).map(P)
    let C
    try {
      const i = o.slice(0, -1).reduce((Q, B) => Q[B], A),
        E = o.reduce((Q, B) => Q[B], A)
      switch (r) {
        case 'GET':
          C = E
          break
        case 'SET':
          ;((i[o.slice(-1)[0]] = P(g.data.value)), (C = !0))
          break
        case 'APPLY':
          C = E.apply(i, c)
          break
        ...
        case 'ENDPOINT':
          {
            const { port1: Q, port2: B } = new MessageChannel()
            ;(OA(A, B), (C = pe(Q, [Q])))
          }
          break
        case 'RELEASE':
    ...
    Promise.resolve(C)
      .catch((i) => ({ value: i, [QA]: 0 }))
      .then((i) => {
        const [E, Q] = DA(i)
        ;(e.postMessage(Object.assign(Object.assign({}, E), { id: n }), Q),
          r === 'RELEASE' &&
            (e.removeEventListener('message', I),
            Se(e),
            mA in A && typeof A[mA] == 'function' && A[mA]()))
      })
      .catch((i) => {
        const [E, Q] = DA({ value: new TypeError('Unserializable return value'), [QA]: 0 })
        e.postMessage(Object.assign(Object.assign({}, E), { id: n }), Q)
      })
  }),
    e.start && e.start())
...
function Fe(A) {
  return Z(A, new Map(), { type: 'RELEASE' }).then(() => {
    Se(A)
  })
}
const fA = new WeakMap(),
  uA =
    ...
    new FinalizationRegistry((A) => {
      const e = (fA.get(A) || 0) - 1
      ;(fA.set(A, e), e === 0 && Fe(A))
    })
function rt(A, e) {
  const t = (fA.get(e) || 0) + 1
  ;(fA.set(e, t), uA && uA.register(A, e, A))
}
function it(A) {
  uA && uA.unregister(A)
}
function bA(A, e, t = [], I = function () {}) {
  let g = !1
  const n = new Proxy(I, {
    get(r, o) {
      if ((CA(g), o === At))
        return () => {
          ;(it(n), Fe(A), e.clear(), (g = !0))
        }
      if (o === 'then') {
        if (t.length === 0) return { then: () => n }
        const c = Z(A, e, { type: 'GET', path: t.map((C) => C.toString()) }).then(P)
    ...
    set(r, o, c) {
      CA(g)
      const [C, i] = DA(c)
      return Z(A, e, { type: 'SET', path: [...t, o].map((E) => E.toString()), value: C }, i).then(P)
    },
    apply(r, o, c) {
      CA(g)
      const C = t[t.length - 1]
      if (C === $e) return Z(A, e, { type: 'ENDPOINT' }).then(P)
      if (C === 'bind') return bA(A, e, t.slice(0, -1))
      const [i, E] = vA(c)
      return Z(A, e, { type: 'APPLY', path: t.map((Q) => Q.toString()), argumentList: i }, E).then(
        P,
      )
    },
    construct(r, o) {
      ...
      return Z(
        A,
        e,
        { type: 'CONSTRUCT', path: t.map((i) => i.toString()), argumentList: c },
        C,
      ).then(P)
    },
  })
...
function vA(A) {
  const e = A.map(DA)
  return [e.map((t) => t[0]), ot(e.map((t) => t[1]))]
}
...
function st(A) {
  return Object.assign(A, { [de]: !0 })
}
function DA(A) {
  for (const [e, t] of we)
    if (t.canHandle(A)) {
      const [I, g] = t.serialize(A)
      return [{ type: 'HANDLER', name: e, value: I }, g]
    }
  return [{ type: 'RAW', value: A }, Re.get(A) || []]
}
function P(A) {
  switch (A.type) {
    case 'HANDLER':
      return we.get(A.name).deserialize(A.value)
    case 'RAW':
      return A.value
  }
}
function Z(A, e, t, I) {
  return new Promise((g) => {
    const n = Ct()
    ;(e.set(n, g), A.start && A.start(), A.postMessage(Object.assign({ id: n }, t), I))
  })
}
function Ct() {
  return new Array(4)
...
const { toString: Bt } = Object.prototype,
  ...
  { iterator: dA, toStringTag: Ne } = Symbol,
  yA = ((A) => (e) => {
    const t = Bt.call(e)
    return A[t] || (A[t] = t.slice(8, -1).toLowerCase())
  })(Object.create(null)),
  J = (A) => ((A = A.toLowerCase()), (e) => yA(e) === A),
  wA = (A) => (e) => typeof e === A,
  { isArray: X } = Array,
  V = wA('undefined')
function tA(A) {
...
const Qt = wA('string'),
  N = wA('function'),
  Ge = wA('number'),
  IA = (A) => A !== null && typeof A == 'object',
  Et = (A) => A === !0 || A === !1,
  EA = (A) => {
    if (yA(A) !== 'object') return !1
    const e = KA(A)
    return (
  ...
  lt = J('Date'),
  ft = J('File'),
  ut = (A) => !!(A && typeof A.uri < 'u'),
  Dt = (A) => A && typeof A.getParts < 'u',
  ht = J('Blob'),
  dt = J('FileList'),
  yt = (A) => IA(A) && N(A.pipe)
function wt() {
...
const $A = wt(),
  Ae = typeof $A.FormData < 'u' ? $A.FormData : void 0,
  St = (A) => {
    let e
    return (
      A &&
      ((Ae && A instanceof Ae) ||
        (N(A.append) &&
          ((e = yA(A)) === 'formdata' ||
            (e === 'object' && N(A.toString) && A.toString() === '[object FormData]'))))
    )
  },
  Ft = J('URLSearchParams'),
  [Rt, pt, mt, Nt] = ['ReadableStream', 'Request', 'Response', 'Headers'].map(J),
  Ut = (A) => (A.trim ? A.trim() : A.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, ''))
function gA(A, e, { allOwnKeys: t = !1 } = {}) {
  if (A === null || typeof A > 'u') return
  let I, g
  if ((typeof A != 'object' && (A = [A]), X(A)))
    for (I = 0, g = A.length; I < g; I++) e.call(null, A[I], I, A)
  else {
    if (tA(A)) return
    const n = t ? Object.getOwnPropertyNames(A) : Object.keys(A),
      r = n.length
    let o
    for (I = 0; I < r; I++) ((o = n[I]), e.call(null, A[o], o, A))
  }
}
function ke(A, e) {
  if (tA(A)) return null
  ...
  let I = t.length,
    g
  for (; I-- > 0; ) if (((g = t[I]), e === g.toLowerCase())) return g
  return null
}
const x =
    typeof globalThis < 'u'
  ...
  Le = (A) => !V(A) && A !== x
function JA() {
  const { caseless: A, skipUndefined: e } = (Le(this) && this) || {},
    t = {},
    I = (g, n) => {
  ...
  for (let g = 0, n = arguments.length; g < n; g++) arguments[g] && gA(arguments[g], I)
  return t
}
const Gt = (A, e, t, { allOwnKeys: I } = {}) => (
    gA(
      e,
      (g, n) => {
        t && N(g)
  ...
  kt = (A) => (A.charCodeAt(0) === 65279 && (A = A.slice(1)), A),
  Lt = (A, e, t, I) => {
  ...
  bt = (A, e, t, I) => {
    ...
    do {
      for (g = Object.getOwnPropertyNames(A), n = g.length; n-- > 0; )
        ((r = g[n]), (!I || I(r, A, e)) && !o[r] && ((e[r] = A[r]), (o[r] = !0)))
      A = t !== !1 && KA(A)
  ...
  Mt = (A, e) => {
    let t
    const I = []
    for (; (t = A.exec(e)) !== null; ) I.push(t)
    return I
  },
  Ot = J('HTMLFormElement'),
  Kt = (A) =>
    A.toLowerCase().replace(/[-_\s]([a-z\d])(\w*)/g, function (t, I, g) {
  ...
  Pt = (A) => {
    be(A, (e, t) => {
      if (N(A) && ['arguments', 'caller', 'callee'].indexOf(t) !== -1) return !1
      const I = A[t]
      if (N(I)) {
  ...
  xt = (A, e) => {
    ...
    return (X(A) ? I(A) : I(String(A).split(e)), t)
  },
  jt = () => {},
  Wt = (A, e) => (A != null && Number.isFinite((A = +A)) ? A : e)
function _t(A) {
  return !!(A && N(A.append) && A[Ne] === 'FormData' && A[dA])
}
const Zt = (A) => {
  ...
  Vt = J('AsyncFunction'),
  Xt = (A) => A && (IA(A) || N(A)) && N(A.then) && N(A.catch),
  Je = ((A, e) =>
    A
      ? setImmediate
      : e
        ? ((t, I) => (
            x.addEventListener(
              'message',
              ({ source: g, data: n }) => {
                g === x && n === t && I.length && I.shift()()
              },
              !1,
            ),
            (g) => {
              ;(I.push(g), x.postMessage(t, '*'))
            }
          ))(`axios@${Math.random()}`, [])
        : (t) => setTimeout(t))(typeof setImmediate == 'function', N(x.postMessage)),
  zt =
...
let u = class He extends Error {
  static from(e, t, I, g, n, r) {
    const o = new He(e.message, t || e.code, I, g, n)
    return (
      (o.cause = e),
      (o.name = e.name),
      e.status != null && o.status == null && (o.status = e.status),
      r && Object.assign(o, r),
      o
    )
  }
  constructor(e, t, I, g, n) {
    ;(super(e),
      ...
      I && (this.config = I),
      g && (this.request = g),
      n && ((this.response = n), (this.status = n.status)))
  }
  toJSON() {
    return {
...
var $t = null
function HA(A) {
  return s.isPlainObject(A) || s.isArray(A)
}
function Ye(A) {
  return s.endsWith(A, '[]') ? A.slice(0, -2) : A
}
function NA(A, e, t) {
  return A
    ? A.concat(e)
        .map(function (g, n) {
          return ((g = Ye(g)), !t && n ? '[' + g + ']' : g)
        })
        .join(t ? '.' : '')
    : e
}
function AI(A) {
  return s.isArray(A) && !A.some(HA)
}
const eI = s.toFlatObject(s, {}, null, function (e) {
  return /^is[A-Z]/.test(e)
})
function SA(A, e, t) {
  if (!s.isObject(A)) throw new TypeError('target must be an object')
  ;((e = e || new FormData()),
    (t = s.toFlatObject(t, { metaTokens: !0, dots: !1, indexes: !1 }, !1, function (l, f) {
      return !s.isUndefined(f[l])
    })))
  const I = t.metaTokens,
    g = t.visitor || i,
    n = t.dots,
    r = t.indexes,
    c = (t.Blob || (typeof Blob < 'u' && Blob)) && s.isSpecCompliantForm(e)
  if (!s.isFunction(g)) throw new TypeError('visitor must be a function')
  function C(a) {
    if (a === null) return ''
    if (s.isDate(a)) return a.toISOString()
    if (s.isBoolean(a)) return a.toString()
    if (!c && s.isBlob(a)) throw new u('Blob is not supported. Use a Buffer instead.')
  ...
  function i(a, l, f) {
    let D = a
    if (s.isReactNative(e) && s.isReactNativeBlob(a)) return (e.append(NA(f, l, n), C(a)), !1)
    if (a && !f && typeof a == 'object') {
      if (s.endsWith(l, '{}')) ((l = I ? l : l.slice(0, -2)), (a = JSON.stringify(a)))
      else if (
        (s.isArray(a) && AI(a)) ||
        ((s.isFileList(a) || s.endsWith(l, '[]')) && (D = s.toArray(a)))
      )
        return (
          (l = Ye(l)),
          D.forEach(function (d, S) {
            !(s.isUndefined(d) || d === null) &&
              e.append(r === !0 ? NA([l], S, n) : r === null ? l : l + '[]', C(d))
          }),
          !1
        )
    }
    return HA(a) ? !0 : (e.append(NA(f, l, n), C(a)), !1)
  }
  const E = [],
    Q = Object.assign(eI, { defaultVisitor: i, convertValue: C, isVisitable: HA })
  function B(a, l) {
    if (!s.isUndefined(a)) {
      if (E.indexOf(a) !== -1) throw Error('Circular reference detected in ' + l.join('.'))
      ;(E.push(a),
        s.forEach(a, function (D, R) {
          ;(!(s.isUndefined(D) || D === null) &&
            g.call(e, D, s.isString(R) ? R.trim() : R, l, Q)) === !0 && B(D, l ? l.concat(R) : [R])
        }),
        E.pop())
    }
  }
  if (!s.isObject(A)) throw new TypeError('data must be an object')
  return (B(A), e)
}
function te(A) {
...
function TA(A, e) {
  ;((this._pairs = []), A && SA(A, this, e))
}
const qe = TA.prototype
...
function Me(A, e, t) {
  if (!e) return A
  const I = (t && t.encode) || tI,
    g = s.isFunction(t) ? { serialize: t } : t,
    n = g && g.serialize
  let r
  if (
    (n ? (r = n(e, g)) : (r = s.isURLSearchParams(e) ? e.toString() : new TA(e, g).toString(I)), r)
  ) {
    const o = A.indexOf('#')
    ;(o !== -1 && (A = A.slice(0, o)), (A += (A.indexOf('?') === -1 ? '?' : '&') + r))
  }
  return A
}
class Ie {
...
var PA = {
  ...
  II = typeof URLSearchParams < 'u' ? URLSearchParams : TA,
  gI = typeof FormData < 'u' ? FormData : null,
  nI = typeof Blob < 'u' ? Blob : null,
  rI = {
    isBrowser: !0,
    classes: { URLSearchParams: II, FormData: gI, Blob: nI },
    protocols: ['http', 'https', 'file', 'blob', 'url', 'data'],
  }
const xA = typeof window < 'u' && typeof document < 'u',
  YA = (typeof navigator == 'object' && navigator) || void 0,
  iI = xA && (!YA || ['ReactNative', 'NativeScript', 'NS'].indexOf(YA.product) < 0),
  oI =
    typeof WorkerGlobalScope < 'u' &&
    self instanceof WorkerGlobalScope &&
    typeof self.importScripts == 'function',
  sI = (xA && window.location.href) || 'http://localhost'
var CI = Object.freeze({
  ...
  p = { ...CI, ...rI }
function BI(A, e) {
  return SA(A, new p.classes.URLSearchParams(), {
    visitor: function (t, I, g, n) {
      return p.isNode && s.isBuffer(t)
...
function aI(A) {
  return s.matchAll(/\w+|\[(\w*)]/g, A).map((e) => (e[0] === '[]' ? '' : e[1] || e[0]))
}
function QI(A) {
  const e = {},
    t = Object.keys(A)
...
function Oe(A) {
  function e(t, I, g, n) {
    ...
    const o = Number.isFinite(+r),
      c = n >= t.length
    return (
      (r = !r && s.isArray(g) ? g.length : r),
      c
        ? (s.hasOwnProp(g, r) ? (g[r] = [g[r], I]) : (g[r] = I), !o)
        : ((!g[r] || !s.isObject(g[r])) && (g[r] = []),
          e(t, I, g[r], n) && s.isArray(g[r]) && (g[r] = QI(g[r])),
          !o)
    )
  }
  if (s.isFormData(A) && s.isFunction(A.entries)) {
    const t = {}
    return (
      s.forEachEntry(A, (I, g) => {
        e(aI(I), g, t, 0)
      }),
      t
    )
...
function EI(A, e, t) {
  if (s.isString(A))
    try {
      return ((e || JSON.parse)(A), s.trim(A))
    } catch (I) {
  ...
  transformRequest: [
    function (e, t) {
      const I = t.getContentType() || '',
        ...
        n = s.isObject(e)
      if ((n && s.isHTMLForm(e) && (e = new FormData(e)), s.isFormData(e)))
        return g ? JSON.stringify(Oe(e)) : e
      if (
        s.isArrayBuffer(e) ||
        s.isBuffer(e) ||
        s.isStream(e) ||
      ...
      if (n) {
        if (I.indexOf('application/x-www-form-urlencoded') > -1)
          return BI(e, this.formSerializer).toString()
        if ((o = s.isFileList(e)) || I.indexOf('multipart/form-data') > -1) {
          const c = this.env && this.env.FormData
          return SA(o ? { 'files[]': e } : e, c && new c(), this.formSerializer)
        }
      }
      return n || g ? (t.setContentType('application/json', !1), EI(e)) : e
    },
  ],
  transformResponse: [
    function (e) {
      ...
      if (s.isResponse(e) || s.isReadableStream(e)) return e
      if (e && s.isString(e) && ((I && !this.responseType) || g)) {
        const r = !(t && t.silentJSONParsing) && g
        try {
...
s.forEach(['delete', 'get', 'head', 'post', 'put', 'patch'], (A) => {
  nA.headers[A] = {}
})
...
`,
        ...
        ;((g = r.indexOf(':')),
          (t = r.substring(0, g).trim().toLowerCase()),
          (I = r.substring(g + 1).trim()),
          !(!t || (e[t] && cI[t])) &&
            (t === 'set-cookie'
              ? e[t]
                ? e[t].push(I)
...
function cA(A) {
  return A === !1 || A == null ? A : s.isArray(A) ? A.map(cA) : String(A).replace(/[\r\n]+$/, '')
}
function fI(A) {
  const e = Object.create(null),
    t = /([^\s,;=]+)\s*(?:=\s*([^,;]+))?/g
  let I
  for (; (I = t.exec(A)); ) e[I[1]] = I[2]
  return e
}
const uI = (A) => /^[-_a-zA-Z0-9^`|~,!#$%&'*+.]+$/.test(A.trim())
function UA(A, e, t, I, g) {
  if (s.isFunction(I)) return I.call(this, e, t)
  if ((g && (e = t), !!s.isString(e))) {
    if (s.isString(I)) return e.indexOf(I) !== -1
    if (s.isRegExp(I)) return I.test(e)
  }
}
function DI(A) {
  return A.trim()
...
function hI(A, e) {
  const t = s.toCamelCase(' ' + e)
  ;['get', 'set', 'has'].forEach((I) => {
    Object.defineProperty(A, I + t, {
      value: function (g, n, r) {
        return this[I].call(this, e, g, n, r)
      },
      configurable: !0,
    })
  })
...
let U = class {
  ...
  set(e, t, I) {
    ...
    function n(o, c, C) {
      ...
      const E = s.findKey(g, i)
      ;(!E || g[E] === void 0 || C === !0 || (C === void 0 && g[E] !== !1)) && (g[E || c] = cA(o))
    }
    const r = (o, c) => s.forEach(o, (C, i) => n(C, i, c))
    if (s.isPlainObject(e) || e instanceof this.constructor) r(e, t)
    else if (s.isString(e) && (e = e.trim()) && !uI(e)) r(lI(e), t)
    else if (s.isObject(e) && s.isIterable(e)) {
      let o = {},
        c,
        C
      for (const i of e) {
        if (!s.isArray(i)) throw TypeError('Object iterator must return a key-value pair')
        o[(C = i[0])] = (c = o[C]) ? (s.isArray(c) ? [...c, i[1]] : [c, i[1]]) : i[1]
      }
      r(o, t)
    } else e != null && n(t, e, I)
    return this
  ...
  get(e, t) {
    if (((e = v(e)), e)) {
      const I = s.findKey(this, e)
      if (I) {
        const g = this[I]
        if (!t) return g
        if (t === !0) return fI(g)
        if (s.isFunction(t)) return t.call(this, g, I)
        if (s.isRegExp(t)) return t.exec(g)
  ...
  has(e, t) {
    if (((e = v(e)), e)) {
      const I = s.findKey(this, e)
      return !!(I && this[I] !== void 0 && (!t || UA(this, this[I], I, t)))
    }
    return !1
  }
  delete(e, t) {
    ...
    function n(r) {
      if (((r = v(r)), r)) {
        const o = s.findKey(I, r)
        o && (!t || UA(I, I[o], o, t)) && (delete I[o], (g = !0))
      }
    }
    return (s.isArray(e) ? e.forEach(n) : n(e), g)
  }
  clear(e) {
    ...
    for (; I--; ) {
      const n = t[I]
      ;(!e || UA(this, this[n], n, e, !0)) && (delete this[n], (g = !0))
    }
    return g
  }
  normalize(e) {
    ...
    return (
      s.forEach(this, (g, n) => {
        const r = s.findKey(I, n)
        if (r) {
          ;((t[r] = cA(g)), delete t[n])
          return
        }
        const o = e ? DI(n) : String(n).trim()
        ;(o !== n && delete t[n], (t[o] = cA(g)), (I[o] = !0))
      }),
      this
    )
  }
  ...
  toJSON(e) {
    ...
    return (
      s.forEach(this, (I, g) => {
        I != null && I !== !1 && (t[g] = e && s.isArray(I) ? I.join(', ') : I)
      }),
      t
    )
  }
...
`)
  ...
  static concat(e, ...t) {
    const I = new this(e)
    return (t.forEach((g) => I.set(g)), I)
  }
  static accessor(e) {
    const I = (this[ge] = this[ge] = { accessors: {} }).accessors,
      g = this.prototype
    function n(r) {
      const o = v(r)
      I[o] || (hI(g, r), (I[o] = !0))
    }
    return (s.isArray(e) ? e.forEach(n) : n(e), this)
  }
}
U.accessor([
  'Content-Type',
...
s.reduceDescriptors(U.prototype, ({ value: A }, e) => {
...
function GA(A, e) {
  ...
  let n = I.data
  return (
    s.forEach(A, function (o) {
      n = o.call(t, n, g.normalize(), e ? e.status : void 0)
    }),
    g.normalize(),
    n
  )
...
let rA = class extends u {
  constructor(e, t, I) {
    ;(super(e ?? 'canceled', u.ERR_CANCELED, t, I),
  ...
  return (
    ...
    function (c) {
      ...
      let E = n,
        Q = 0
      for (; E !== g; ) ((Q += t[E++]), (E = E % A))
      if (((g = (g + 1) % A), g === n && (n = (n + 1) % A), C - r < e)) return
      const B = i && C - i
      return B ? Math.round((Q * 1e3) / B) : void 0
    }
  )
}
function wI(A, e) {
  ...
  const r = (C, i = Date.now()) => {
    ;((t = i), (g = null), n && (clearTimeout(n), (n = null)), A(...C))
  }
  return [
    (...C) => {
      const i = Date.now(),
...
const hA = (A, e, t = 3) => {
  ...
  ne = (A, e) => {
    const t = A != null
    return [(I) => e[0]({ lengthComputable: t, total: A, loaded: I }), e[1]]
  },
  re =
    (A) =>
    (...e) =>
      s.asap(() => A(...e))
var SI = p.hasStandardBrowserEnv
    ? ((A, e) => (t) => (
        (t = new URL(t, p.origin)),
        A.protocol === t.protocol && A.host === t.host && (e || A.port === t.port)
      ))(new URL(p.origin), p.navigator && /(msie|trident)/i.test(p.navigator.userAgent))
    : () => !0,
  FI = p.hasStandardBrowserEnv
    ? {
        write(A, e, t, I, g, n, r) {
          if (typeof document > 'u') return
          const o = [`${A}=${encodeURIComponent(e)}`]
          ;(s.isNumber(t) && o.push(`expires=${new Date(t).toUTCString()}`),
            s.isString(I) && o.push(`path=${I}`),
            s.isString(g) && o.push(`domain=${g}`),
            n === !0 && o.push('secure'),
            s.isString(r) && o.push(`SameSite=${r}`),
...
function Pe(A, e, t) {
  let I = !RI(e)
  return A && (I || t == !1) ? pI(A, e) : e
}
const ie = (A) => (A instanceof U ? { ...A } : A)
function W(A, e) {
  e = e || {}
  const t = {}
  function I(C, i, E, Q) {
    return s.isPlainObject(C) && s.isPlainObject(i)
      ? s.merge.call({ caseless: Q }, C, i)
      : s.isPlainObject(i)
        ? s.merge({}, i)
        : s.isArray(i)
          ? i.slice()
  ...
  function g(C, i, E, Q) {
    if (s.isUndefined(i)) {
      if (!s.isUndefined(C)) return I(void 0, C, E, Q)
    } else return I(C, i, E, Q)
  }
  function n(C, i) {
    if (!s.isUndefined(i)) return I(void 0, i)
  }
  function r(C, i) {
    if (s.isUndefined(i)) {
  ...
  function o(C, i, E) {
    if (E in e) return I(C, i)
    if (E in A) return I(void 0, C)
  }
  const c = {
    url: n,
    method: n,
    ...
    responseEncoding: r,
    validateStatus: o,
    headers: (C, i, E) => g(ie(C), ie(i), E, !0),
  }
  return (
    s.forEach(Object.keys({ ...A, ...e }), function (i) {
      if (i === '__proto__' || i === 'constructor' || i === 'prototype') return
      const E = s.hasOwnProp(c, i) ? c[i] : g,
        Q = E(A[i], e[i], i)
      ;(s.isUndefined(Q) && E !== o) || (t[i] = Q)
    }),
    t
  )
}
var xe = (A) => {
  const e = W({}, A)
  let { data: t, withXSRFToken: I, xsrfHeaderName: g, xsrfCookieName: n, headers: r, auth: o } = e
  if (
    ((e.headers = r = U.from(r)),
    (e.url = Me(Pe(e.baseURL, e.url, e.allowAbsoluteUrls), A.params, A.paramsSerializer)),
    o &&
      r.set(
        'Authorization',
        'Basic ' +
    ...
    if (p.hasStandardBrowserEnv || p.hasStandardBrowserWebWorkerEnv) r.setContentType(void 0)
    else if (s.isFunction(t.getHeaders)) {
      const c = t.getHeaders(),
        C = ['content-type', 'content-length']
      Object.entries(c).forEach(([i, E]) => {
        C.includes(i.toLowerCase()) && r.set(i, E)
      })
    }
  }
  if (
    p.hasStandardBrowserEnv &&
    (I && s.isFunction(I) && (I = I(e)), I || (I !== !1 && SI(e.url)))
  ) {
    const c = g && n && FI.read(n)
    c && r.set(g, c)
  }
  return e
}
const mI = typeof XMLHttpRequest < 'u'
var NI =
  mI &&
  function (A) {
    return new Promise(function (t, I) {
      const g = xe(A)
      let n = g.data
      const r = U.from(g.headers).normalize()
      let { responseType: o, onUploadProgress: c, onDownloadProgress: C } = g,
        i,
        E,
        Q,
        B,
      ...
      function l() {
        ;(B && B(),
          a && a(),
          g.cancelToken && g.cancelToken.unsubscribe(i),
          g.signal && g.signal.removeEventListener('abort', i))
      }
      let f = new XMLHttpRequest()
      ;(f.open(g.method.toUpperCase(), g.url, !0), (f.timeout = g.timeout))
      function D() {
        if (!f) return
        const d = U.from('getAllResponseHeaders' in f && f.getAllResponseHeaders()),
          b = {
      ...
      ;('onloadend' in f
        ...
        (f.onabort = function () {
          f && (I(new u('Request aborted', u.ECONNABORTED, A, f)), (f = null))
        }),
        (f.onerror = function (S) {
          const b = S && S.message ? S.message : 'Network Error',
            K = new u(b, u.ERR_NETWORK, A, f)
          ;((K.event = S || null), I(K), (f = null))
        }),
        (f.ontimeout = function () {
          let S = g.timeout ? 'timeout of ' + g.timeout + 'ms exceeded' : 'timeout exceeded'
          const b = g.transitional || PA
          ;(g.timeoutErrorMessage && (S = g.timeoutErrorMessage),
            I(new u(S, b.clarifyTimeoutError ? u.ETIMEDOUT : u.ECONNABORTED, A, f)),
            (f = null))
        }),
        n === void 0 && r.setContentType(null),
        'setRequestHeader' in f &&
          s.forEach(r.toJSON(), function (S, b) {
            f.setRequestHeader(b, S)
          }),
        s.isUndefined(g.withCredentials) || (f.withCredentials = !!g.withCredentials),
        o && o !== 'json' && (f.responseType = g.responseType),
        C && (([Q, a] = hA(C, !0)), f.addEventListener('progress', Q)),
        c &&
          f.upload &&
          (([E, B] = hA(c)),
          f.upload.addEventListener('progress', E),
          f.upload.addEventListener('loadend', B)),
        (g.cancelToken || g.signal) &&
          ((i = (d) => {
            f && (I(!d || d.type ? new rA(null, A, f) : d), f.abort(), (f = null))
          }),
          g.cancelToken && g.cancelToken.subscribe(i),
          g.signal && (g.signal.aborted ? i() : g.signal.addEventListener('abort', i))))
      const R = dI(g.url)
      if (R && p.protocols.indexOf(R) === -1) {
        I(new u('Unsupported protocol ' + R + ':', u.ERR_BAD_REQUEST, A))
        return
      }
      f.send(n || null)
    })
...
const UI = (A, e) => {
    ...
    if (e || t) {
      ...
      const n = function (C) {
        if (!g) {
          ...
          I.abort(i instanceof u ? i : new rA(i instanceof Error ? i.message : i))
        }
      }
      let r =
        e &&
        setTimeout(() => {
          ;((r = null), n(new u(`timeout of ${e}ms exceeded`, u.ETIMEDOUT)))
        }, e)
      const o = () => {
        A &&
          (r && clearTimeout(r),
      ...
      const { signal: c } = I
      return ((c.unsubscribe = () => s.asap(o)), c)
    }
  },
  GI = function* (A, e) {
    let t = A.byteLength
    ...
    let I = 0,
      g
    for (; I < t; ) ((g = I + e), yield A.slice(I, g), (I = g))
  },
  kI = async function* (A, e) {
    for await (const t of LI(A)) yield* GI(t, e)
  },
  LI = async function* (A) {
    ...
    return new ReadableStream(
      {
        async pull(c) {
          try {
            const { done: C, value: i } = await g.next()
            if (C) {
              ;(o(), c.close())
              return
            }
  ...
  se = 64 * 1024,
  { isFunction: BA } = s,
  bI = (({ Request: A, Response: e }) => ({ Request: A, Response: e }))(s.global),
  { ReadableStream: Ce, TextEncoder: Be } = s.global,
  ae = (A, ...e) => {
    try {
      return !!A(...e)
    } catch {
  ...
  JI = (A) => {
    A = s.merge.call({ skipUndefined: !0 }, bI, A)
    const { fetch: e, Request: t, Response: I } = A,
      g = e ? BA(e) : typeof fetch == 'function',
      n = BA(t),
      r = BA(I)
    if (!g) return !1
    const o = g && BA(Ce),
      c =
        ...
        (typeof Be == 'function'
          ...
          : async (a) => new Uint8Array(await new t(a).arrayBuffer())),
      C =
        n &&
        o &&
        ae(() => {
      ...
      i = r && o && ae(() => s.isReadableStream(new I('').body)),
      E = { stream: i && ((a) => a.body) }
    g &&
      ['text', 'arrayBuffer', 'blob', 'formData', 'stream'].forEach((a) => {
        !E[a] &&
          (E[a] = (l, f) => {
            ...
            throw new u(`Response type '${a}' is not supported`, u.ERR_NOT_SUPPORT, f)
          })
      })
    const Q = async (a) => {
        if (a == null) return 0
        if (s.isBlob(a)) return a.size
        if (s.isSpecCompliantForm(a))
          return (await new t(p.origin, { method: 'POST', body: a }).arrayBuffer()).byteLength
        if (s.isArrayBufferView(a) || s.isArrayBuffer(a)) return a.byteLength
        if ((s.isURLSearchParams(a) && (a = a + ''), s.isString(a))) return (await c(a)).byteLength
      },
      B = async (a, l) => {
        const f = s.toFiniteNumber(a.getContentLength())
        return f ?? Q(l)
      }
    return async (a) => {
      let {
          ...
          responseType: G,
          headers: RA,
          withCredentials: iA = 'same-origin',
          fetchOptions: WA,
        } = xe(a),
        _A = e || fetch
      G = G ? (G + '').toLowerCase() : 'text'
      let oA = UI([R, d && d.toAbortSignal()], S),
        z = null
      const T =
        oA &&
        oA.unsubscribe &&
      ...
      let ZA
      try {
        if (K && C && f !== 'get' && f !== 'head' && (ZA = await B(RA, D)) !== 0) {
          let M = new t(l, { method: 'POST', body: D, duplex: 'half' }),
            _
          if (
            (s.isFormData(D) && (_ = M.headers.get('content-type')) && RA.setContentType(_), M.body)
          ) {
            const [pA, sA] = ne(ZA, hA(re(K)))
            D = oe(M.body, se, pA, sA)
          }
        }
        s.isString(iA) || (iA = iA ? 'include' : 'omit')
        const m = n && 'credentials' in t.prototype,
          VA = {
            ...WA,
            signal: oA,
            method: f.toUpperCase(),
            headers: RA.normalize().toJSON(),
            body: D,
            duplex: 'half',
            credentials: m ? iA : void 0,
          }
        z = n && new t(l, VA)
        let q = await (n ? _A(z, WA) : _A(l, VA))
        const XA = i && (G === 'stream' || G === 'response')
        if (i && (b || (XA && T))) {
          const M = {}
          ;['status', 'statusText', 'headers'].forEach((zA) => {
            M[zA] = q[zA]
          })
          const _ = s.toFiniteNumber(q.headers.get('content-length')),
            [pA, sA] = (b && ne(_, hA(re(b), !0))) || []
          q = new I(
            oe(q.body, se, pA, () => {
              ;(sA && sA(), T && T())
            }),
        ...
        G = G || 'text'
        let ve = await E[s.findKey(E, G) || 'text'](q, a)
        return (
          !XA && T && T(),
        ...
        throw (
          ...
          m && m.name === 'TypeError' && /Load failed|fetch/i.test(m.message)
            ? Object.assign(new u('Network Error', u.ERR_NETWORK, a, z, m && m.response), {
                cause: m.cause || m,
              })
            : u.from(m, m && m.code, a, z, m && m.response)
        )
      }
    }
  },
  ...
  je = (A) => {
    let e = (A && A.env) || {}
    const { fetch: t, Request: I, Response: g } = e,
      n = [I, g, t]
    let r = n.length,
      o = r,
      c,
      ...
      i = HI
    for (; o--; )
      ((c = n[o]), (C = i.get(c)), C === void 0 && i.set(c, (C = o ? new Map() : JI(e))), (i = C))
    return C
  }
je()
const jA = { http: $t, xhr: NI, fetch: { get: je } }
s.forEach(jA, (A, e) => {
  if (A) {
    try {
...
const Qe = (A) => `- ${A}`,
  YI = (A) => s.isFunction(A) || A === null || A === !1
function qI(A, e) {
  A = s.isArray(A) ? A : [A]
  const { length: t } = A
  let I, g
  ...
  for (let r = 0; r < t; r++) {
    I = A[r]
    let o
    if (((g = I), !YI(I) && ((g = jA[(o = String(I)).toLowerCase()]), g === void 0)))
      throw new u(`Unknown adapter '${o}'`)
    if (g && (s.isFunction(g) || (g = g.get(e)))) break
    n[o || '#' + r] = g
  }
  if (!g) {
...
`)
    ...
    throw new u('There is no suitable adapter to dispatch the request ' + o, 'ERR_NOT_SUPPORT')
  }
  return g
}
var We = { getAdapter: qI, adapters: jA }
function kA(A) {
...
function Ee(A) {
  return (
    kA(A),
    (A.headers = U.from(A.headers)),
    (A.data = GA.call(A, A.transformRequest)),
    ['post', 'put', 'patch'].indexOf(A.method) !== -1 &&
      A.headers.setContentType('application/x-www-form-urlencoded', !1),
    We.getAdapter(
      A.adapter || nA.adapter,
      A,
    )(A).then(
      function (I) {
        return (
          kA(A),
          (I.data = GA.call(A, A.transformResponse, I)),
          (I.headers = U.from(I.headers)),
          I
        )
      },
      function (I) {
        return (
          Ke(I) ||
            ...
            I &&
              I.response &&
              ((I.response.data = GA.call(A, A.transformResponse, I.response)),
              (I.response.headers = U.from(I.response.headers)))),
          Promise.reject(I)
        )
      },
    )
...
const ce = {}
FA.transitional = function (e, t, I) {
  function g(n, r) {
    return '[Axios v' + _e + "] Transitional option '" + n + "'" + r + (I ? '. ' + I : '')
  }
  return (n, r, o) => {
    if (e === !1) throw new u(g(r, ' has been removed' + (t ? ' in ' + t : '')), u.ERR_DEPRECATED)
    return (
      t &&
        !ce[r] &&
        ((ce[r] = !0),
        console.warn(
          g(r, ' has been deprecated since v' + t + ' and will be removed in the near future'),
        )),
      e ? e(n, r, o) : !0
    )
  }
...
function MI(A, e, t) {
  if (typeof A != 'object') throw new u('options must be an object', u.ERR_BAD_OPTION_VALUE)
  ...
  for (; g-- > 0; ) {
    ...
    if (r) {
      const o = A[n],
        c = o === void 0 || r(o, n, A)
      if (c !== !0) throw new u('option ' + n + ' must be ' + c, u.ERR_BAD_OPTION_VALUE)
      continue
    }
    if (t !== !0) throw new u('Unknown option ' + n, u.ERR_BAD_OPTION)
  }
}
var lA = { assertOptions: MI, validators: FA }
const k = lA.validators
let j = class {
  constructor(e) {
    ;((this.defaults = e || {}), (this.interceptors = { request: new Ie(), response: new Ie() }))
  }
  async request(e, t) {
    try {
      ...
      if (I instanceof Error) {
        ...
        const n = g.stack ? g.stack.replace(/^.+\n/, '') : ''
        try {
          I.stack
            ? n &&
...
` + n)
  ...
  _request(e, t) {
    ;(typeof e == 'string' ? ((t = t || {}), (t.url = e)) : (t = e || {}),
      (t = W(this.defaults, t)))
    const { transitional: I, paramsSerializer: g, headers: n } = t
    ;(I !== void 0 &&
      lA.assertOptions(
        I,
        {
      ...
      g != null &&
        (s.isFunction(g)
          ? (t.paramsSerializer = { serialize: g })
          : lA.assertOptions(g, { encode: k.function, serialize: k.function }, !0)),
      ...
      lA.assertOptions(
        t,
        { baseUrl: k.spelling('baseURL'), withXsrfToken: k.spelling('withXSRFToken') },
        !0,
      ),
      (t.method = (t.method || this.defaults.method || 'get').toLowerCase()))
    let r = n && s.merge(n.common, n[t.method])
    ;(n &&
      s.forEach(['delete', 'get', 'head', 'post', 'put', 'patch', 'common'], (a) => {
        delete n[a]
      }),
      (t.headers = U.concat(r, n)))
    const o = []
    let c = !0
    this.interceptors.request.forEach(function (l) {
      if (typeof l.runWhen == 'function' && l.runWhen(t) === !1) return
      c = c && l.synchronous
      const f = t.transitional || PA
      f && f.legacyInterceptorReqResOrdering
        ? o.unshift(l.fulfilled, l.rejected)
    ...
    let i,
      ...
      Q
    if (!c) {
      const a = [Ee.bind(this), void 0]
      for (a.unshift(...o), a.push(...C), Q = a.length, i = Promise.resolve(t); E < Q; )
        i = i.then(a[E++], a[E++])
      return i
    }
    Q = o.length
    ...
    for (E = 0, Q = C.length; E < Q; ) i = i.then(C[E++], C[E++])
    return i
  }
  getUri(e) {
    e = W(this.defaults, e)
    const t = Pe(e.baseURL, e.url, e.allowAbsoluteUrls)
    return Me(t, e.params, e.paramsSerializer)
  }
}
s.forEach(['delete', 'get', 'head', 'options'], function (e) {
  j.prototype[e] = function (t, I) {
    return this.request(W(I || {}, { method: e, url: t, data: (I || {}).data }))
  }
})
...
let OI = class Ze {
  constructor(e) {
    ...
    ;(this.promise.then((g) => {
      ...
      e(function (n, r, o) {
        I.reason || ((I.reason = new rA(n, r, o)), t(I.reason))
      }))
  }
  throwIfRequested() {
...
function TI(A) {
  return s.isObject(A) && A.isAxiosError === !0
}
const qA = {
  Continue: 100,
...
Object.entries(qA).forEach(([A, e]) => {
  qA[e] = A
})
function Ve(A) {
  const e = new j(A),
    t = me(j.prototype.request, e)
  return (
    s.extend(t, j.prototype, e, { allOwnKeys: !0 }),
    s.extend(t, e, null, { allOwnKeys: !0 }),
    (t.create = function (g) {
      return Ve(W(A, g))
    }),
    t
...
y.mergeConfig = W
y.AxiosHeaders = U
y.formToJSON = (A) => Oe(s.isHTMLForm(A) ? new FormData(A) : A)
...
const LA = {
  env: {
    emscripten_notify_memory_growth: function (A) {
      MA = new Uint8Array(H.exports.memory.buffer)
    },
  },
}
class PI {
  init() {
    return (
      $ ||
      (typeof fetch < 'u'
        ? ($ = fetch('data:application/wasm;base64,' + le)
            .then((e) => e.arrayBuffer())
            .then((e) => WebAssembly.instantiate(e, LA))
            .then(this._init))
        : ($ = WebAssembly.instantiate(Buffer.from(le, 'base64'), LA).then(this._init)),
      $)
    )
  }
  _init(e) {
    ;((H = e.instance), LA.env.emscripten_notify_memory_growth(0))
  }
  decode(e, t = 0) {
    if (!H) throw new Error('ZSTDDecoder: Await .init() before decoding.')
    const I = e.byteLength,
      g = H.exports.malloc(I)
    ;(MA.set(e, g), (t = t || Number(H.exports.ZSTD_findDecompressedSize(g, I))))
    const n = H.exports.malloc(t),
      r = H.exports.ZSTD_decompress(n, t, g, I),
      o = MA.slice(n, n + r)
    return (H.exports.free(g), H.exports.free(n), o)
  }
}
const le =
  ...
  fe = new PI()
let ue = !1
async function xI(A, e, t) {
  let I = null
  ;(typeof A != 'string' ? (I = A.href) : A.startsWith('http') ? (I = A) : (I = `${e}/${A}`),
    I.endsWith('.js') && (I = I.substring(0, I.length - 3)),
    I.endsWith('.wasm') && (I = I.substring(0, I.length - 5)),
    I.endsWith('.wasm.zst') && (I = I.substring(0, I.length - 9)))
  const g = `${I}.wasm`,
    n = await y.get(`${g}.zst`, { responseType: 'arraybuffer', params: t })
  ue || (await fe.init(), (ue = !0))
  const o = fe.decode(new Uint8Array(n.data)).buffer
  return (await import(`${I}.js`)).default({ wasmBinary: o })
}
const aA = new Map()
async function jI(A, e, t) {
  let I = A,
    ...
    n = null
  return (
    typeof A != 'string' && ((I = new URL(A.href)), (g = I.href)),
    aA.has(g) ? (n = await aA.get(g)) : (aA.set(g, xI(I, e, t)), (n = await aA.get(g))),
    n
  )
}
const h = {
  ...
  eA = { Float32: 'float32', Float64: 'float64' }
function Y(A, e) {
  let t = null
  switch (A) {
    case L.UInt8: {
...
const Xe = typeof globalThis.SharedArrayBuffer == 'function',
  De = new TextEncoder(),
  he = new TextDecoder('utf-8')
function WI(A, e) {
  const t = { flags: 'r' },
    I = A.fs_open(e, t.flags),
    n = A.fs_stat(e).size
  let r = null
  Xe ? (r = new SharedArrayBuffer(n)) : (r = new ArrayBuffer(n))
  const o = new Uint8Array(r)
  return (A.fs_read(I, o, 0, n, 0), A.fs_close(I), o)
}
function ze(A, e, t) {
  let I = null
  Xe ? (I = new SharedArrayBuffer(t)) : (I = new ArrayBuffer(t))
  const g = new Uint8Array(I),
    n = new Uint8Array(A.HEAPU8.buffer, e, t)
  return (g.set(n), g)
}
function w(A, e, t, I) {
  let g = 0
  return (
    e !== null &&
      ((g = A.ccall(
        ...
        [0, t, I, e.buffer.byteLength],
      )),
      A.HEAPU8.set(new Uint8Array(e.buffer), g)),
    g
  )
}
function O(A, e, t) {
  const I = JSON.stringify(e),
    g = A.lengthBytesUTF8(I) + 1,
    n = A.ccall('itk_wasm_input_json_alloc', 'number', ['number', 'number', 'number'], [0, t, g])
  A.stringToUTF8(I, n, g)
}
function F(A, e, t, I) {
  const g = A.ccall(
    ...
    n = A.ccall('itk_wasm_output_array_size', 'number', ['number', 'number', 'number'], [0, e, t]),
    r = ze(A, g, n)
  return Y(I, r.buffer)
}
function AA(A, e) {
  const t = A.ccall('itk_wasm_output_json_address', 'number', ['number', 'number'], [0, e]),
    I = A.UTF8ToString(t)
  return JSON.parse(I)
}
function _I(A, e, t, I) {
  ;(I != null &&
    ...
    I.forEach(function (C, i) {
      ...
      switch (C.type) {
        case h.TextStream: {
          const Q = De.encode(C.data.data),
            B = w(A, Q, i, 0),
            a = { size: Q.buffer.byteLength, data: `data:application/vnd.itk.address,0:${B}` }
          O(A, a, i)
          break
        }
        case h.JsonCompatible: {
          const Q = De.encode(JSON.stringify(C.data)),
            B = w(A, Q, i, 0),
            a = { size: Q.buffer.byteLength, data: `data:application/vnd.itk.address,0:${B}` }
          O(A, a, i)
          break
        ...
        case h.TextFile: {
          A.fs_writeFile(C.data.path, C.data.data)
          break
        }
        case h.BinaryFile: {
          A.fs_writeFile(C.data.path, C.data.data)
        ...
        case h.Image: {
          const Q = C.data,
            B = w(A, Q.data, i, 0),
            a = w(A, Q.direction, i, 1),
            l =
              typeof ((E = Q.metadata) == null ? void 0 : E.entries) < 'u'
                ? Array.from(Q.metadata.entries())
                : [],
            f = {
              ...
              origin: Q.origin,
              spacing: Q.spacing,
              direction: `data:application/vnd.itk.address,0:${a}`,
              size: Q.size,
              data: `data:application/vnd.itk.address,0:${B}`,
              metadata: l,
            }
          O(A, f, i)
          break
        ...
        case h.Mesh: {
          const Q = C.data,
            B = w(A, Q.points, i, 0),
            a = w(A, Q.cells, i, 1),
            l = w(A, Q.pointData, i, 2),
            f = w(A, Q.cellData, i, 3),
            D = {
              meshType: Q.meshType,
              name: Q.name,
              numberOfPoints: Q.numberOfPoints,
              points: `data:application/vnd.itk.address,0:${B}`,
              numberOfCells: Q.numberOfCells,
              cells: `data:application/vnd.itk.address,0:${a}`,
              cellBufferSize: Q.cellBufferSize,
              numberOfPointPixels: Q.numberOfPointPixels,
              pointData: `data:application/vnd.itk.address,0:${l}`,
              numberOfCellPixels: Q.numberOfCellPixels,
              cellData: `data:application/vnd.itk.address,0:${f}`,
            }
          O(A, D, i)
          break
        }
        case h.PointSet: {
          const Q = C.data,
            B = w(A, Q.points, i, 0),
            a = w(A, Q.pointData, i, 1),
            l = {
              pointSetType: Q.pointSetType,
              name: Q.name,
              numberOfPoints: Q.numberOfPoints,
              points: `data:application/vnd.itk.address,0:${B}`,
              numberOfPointPixels: Q.numberOfPointPixels,
              pointData: `data:application/vnd.itk.address,0:${a}`,
            }
          O(A, l, i)
          break
        }
        case h.TransformList: {
          const Q = C.data,
            B = []
          let a = 0
          ;(Q.forEach((l) => {
            let f = '',
              D = ''
            l.transformType.transformParameterization !== 'Composite' &&
              ((f = `data:application/vnd.itk.address,0:${w(A, l.fixedParameters, i, a)}`),
              (a += 1),
              (D = `data:application/vnd.itk.address,0:${w(A, l.parameters, i, a)}`),
              (a += 1))
            const R = {
              transformType: l.transformType,
              numberOfFixedParameters: l.numberOfFixedParameters,
              numberOfParameters: l.numberOfParameters,
              name: l.name,
              inputSpaceName: l.inputSpaceName,
              outputSpaceName: l.outputSpaceName,
              parameters: D,
              fixedParameters: f,
            }
            B.push(R)
        ...
        case h.PolyData: {
          const Q = C.data,
            B = w(A, Q.points, i, 0),
            a = w(A, Q.vertices, i, 1),
            l = w(A, Q.lines, i, 2),
            f = w(A, Q.polygons, i, 3),
            D = w(A, Q.triangleStrips, i, 4),
            R = w(A, Q.pointData, i, 5),
            d = w(A, Q.pointData, i, 6),
            S = {
              polyDataType: Q.polyDataType,
              name: Q.name,
              numberOfPoints: Q.numberOfPoints,
              points: `data:application/vnd.itk.address,0:${B}`,
              verticesBufferSize: Q.verticesBufferSize,
              vertices: `data:application/vnd.itk.address,0:${a}`,
              linesBufferSize: Q.linesBufferSize,
              lines: `data:application/vnd.itk.address,0:${l}`,
              polygonsBufferSize: Q.polygonsBufferSize,
              polygons: `data:application/vnd.itk.address,0:${f}`,
              triangleStripsBufferSize: Q.triangleStripsBufferSize,
              triangleStrips: `data:application/vnd.itk.address,0:${D}`,
              numberOfPointPixels: Q.numberOfPointPixels,
              pointData: `data:application/vnd.itk.address,0:${R}`,
              numberOfCellPixels: Q.numberOfCellPixels,
              cellData: `data:application/vnd.itk.address,0:${d}`,
            }
          O(A, S, i)
          break
        }
  ...
  let n = 0
  try {
    n = A.callMain(e.slice())
  } catch (C) {
    throw (
      typeof C == 'number' &&
        (console.log('Exception while running pipeline:'),
        console.log('stdout:', A.getModuleStdout()),
        console.error('stderr:', A.getModuleStderr()),
        typeof A.getExceptionMessage < 'u'
          ? console.error('exception:', A.getExceptionMessage(C))
  ...
  return (
    t != null &&
      ...
      t.forEach(function (C, i) {
        ...
        switch (C.type) {
          case h.TextStream: {
            const B = A.ccall(
              ...
              l = new Uint8Array(A.HEAPU8.buffer, B, a)
            E = { data: he.decode(l) }
            break
          }
          case h.JsonCompatible: {
          ...
          case h.TextFile: {
            E = { path: C.data.path, data: A.fs_readFile(C.data.path, { encoding: 'utf8' }) }
            break
          }
          case h.BinaryFile: {
            E = { path: C.data.path, data: WI(A, C.data.path) }
            break
          }
          case h.Image: {
            const B = AA(A, i)
            ;((B.data = F(A, i, 0, B.imageType.componentType)),
              (B.direction = F(A, i, 1, eA.Float64)),
              (B.metadata = new Map(B.metadata)),
              (E = B))
            break
          }
          case h.Mesh: {
            const B = AA(A, i)
            ;(B.numberOfPoints > 0
              ? (B.points = F(A, i, 0, B.meshType.pointComponentType))
              : (B.points = Y(B.meshType.pointComponentType, new ArrayBuffer(0))),
              B.numberOfCells > 0
                ? (B.cells = F(A, i, 1, B.meshType.cellComponentType))
                : (B.cells = Y(B.meshType.cellComponentType, new ArrayBuffer(0))),
              B.numberOfPointPixels > 0
                ? (B.pointData = F(A, i, 2, B.meshType.pointPixelComponentType))
                : (B.pointData = Y(B.meshType.pointPixelComponentType, new ArrayBuffer(0))),
              B.numberOfCellPixels > 0
                ? (B.cellData = F(A, i, 3, B.meshType.cellPixelComponentType))
                : (B.cellData = Y(B.meshType.cellPixelComponentType, new ArrayBuffer(0))),
              (E = B))
            break
          }
          case h.PointSet: {
            const B = AA(A, i)
            ;(B.numberOfPoints > 0
              ? (B.points = F(A, i, 0, B.pointSetType.pointComponentType))
              : (B.points = Y(B.pointSetType.pointComponentType, new ArrayBuffer(0))),
              B.numberOfPointPixels > 0
                ? (B.pointData = F(A, i, 1, B.pointSetType.pointPixelComponentType))
                : (B.pointData = Y(B.pointSetType.pointPixelComponentType, new ArrayBuffer(0))),
              (E = B))
            break
          }
          case h.TransformList: {
            ...
            ;(B.forEach((l, f) => {
              l.transformType.transformParameterization !== 'Composite' &&
                (l.numberOfFixedParameters > 0 && (B[f].fixedParameters = F(A, i, a, eA.Float64)),
                (a += 1),
                l.numberOfParameters > 0 &&
                  (B[f].parameters = F(A, i, a, l.transformType.parametersValueType)),
                (a += 1))
            }),
              (E = B))
            break
          ...
          case h.PolyData: {
            const B = AA(A, i)
            ;(B.numberOfPoints > 0
              ? (B.points = F(A, i, 0, eA.Float32))
              : (B.points = new Float32Array()),
              B.verticesBufferSize > 0
                ? (B.vertices = F(A, i, 1, L.UInt32))
                : (B.vertices = new Uint32Array()),
              B.linesBufferSize > 0
                ? (B.lines = F(A, i, 2, L.UInt32))
                : (B.lines = new Uint32Array()),
              B.polygonsBufferSize > 0
                ? (B.polygons = F(A, i, 3, L.UInt32))
                : (B.polygons = new Uint32Array()),
              B.triangleStripsBufferSize > 0
                ? (B.triangleStrips = F(A, i, 4, L.UInt32))
                : (B.triangleStrips = new Uint32Array()),
              B.numberOfPointPixels > 0
                ? (B.pointData = F(A, i, 5, B.polyDataType.pointPixelComponentType))
                : (B.pointData = Y(B.polyDataType.pointPixelComponentType, new ArrayBuffer(0))),
              B.numberOfCellPixels > 0
                ? (B.cellData = F(A, i, 6, B.polyDataType.cellPixelComponentType))
                : (B.cellData = Y(B.polyDataType.cellPixelComponentType, new ArrayBuffer(0))),
              (E = B))
            break
          }
          default:
        ...
        const Q = { type: C.type, data: E }
        c.push(Q)
      }),
    { returnValue: n, stdout: r, stderr: o, outputs: c }
  )
}
const ZI = typeof globalThis.SharedArrayBuffer < 'u'
function VI(A, e) {
  if (A == null) return []
  const t = []
...
function XI(A, e) {
  ...
  let t = null
  return (
    A.buffer !== void 0 ? (t = A.buffer) : A.byteLength !== void 0 && (t = A),
    ZI && t instanceof SharedArrayBuffer ? null : t
  )
}
function zI(A) {
  return [A.data, A.direction]
}
function vI(A) {
  return [A.points, A.pointData, A.cells, A.cellData]
}
function $I(A) {
  return [A.points, A.vertices, A.lines, A.polygons, A.triangleStrips, A.pointData, A.cellData]
}
async function Ag(A, e, t, I) {
  const g = _I(A, e, t, I),
    n = []
  return (
    g.outputs.forEach(function (r) {
      if (r.type === h.BinaryStream || r.type === h.BinaryFile) {
        const o = r.data
        n.push(o)
      } else if (r.type === h.Image) {
        const o = r.data
        n.push(...zI(o))
      } else if (r.type === h.Mesh) {
        const o = r.data
        n.push(...vI(o))
      } else if (r.type === h.PolyData) {
        const o = r.data
        n.push(...$I(o))
      }
    }),
...
const eg = {
  runPipeline: async function (A, e, t, I, g, n) {
    const r = await jI(A, e, n)
    return await Ag(r, t, I, g)
  },
}
OA(eg)
